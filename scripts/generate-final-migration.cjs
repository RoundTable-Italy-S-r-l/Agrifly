...
const expectedSchema = {
  // Tabelle da creare
  order_items: {
    columns: ['id', 'order_id', 'sku_id', 'quantity', 'unit_price_cents', 'total_cents'],
    create: true
  },
  organization_settings: {
    columns: ['id', 'org_id', 'setting_key', 'setting_value', 'created_at', 'updated_at'],
    create: true
  },
  ...
  assets: ['productid'],
  locations: ['address_line', 'city', 'province', 'postal_code', 'country', 'latitude', 'longitude', 'is_default'],
  job_offer_messages: [
    'sender_org_id',
...
async function generateFinalMigration() {
  ...
  try {
    ...
    console.log('-- ============================================================');
    console.log('-- Script di migrazione finale - Solo colonne/tabelle mancanti');
    ...
    for (const [tableName, columns] of Object.entries(expectedSchema)) {
      ...
      for (const col of codeColumns) {
        ...
        if (colName === 'message_text' && dbColumns.includes('body')) {
          console.log(`-- ⚠️  ${tableName}.${colName}: esiste 'body' nel DB, ma il codice usa 'message_text'`);
      ...
      if (missingColumns.length > 0) {
        ...
        for (const col of missingColumns) {
          ...
          console.log(`ALTER TABLE ${tableName} ADD COLUMN IF NOT EXISTS ${col.name} ${type};`);
        }
        console.log('');
      }
    }
