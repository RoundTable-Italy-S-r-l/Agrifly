...
const criticalColumns = {
  jobs: ['id', 'buyer_org_id', 'service_type', 'status', 'field_polygon', 'broker_org_id', 'accepted_offer_id', 'visibility_mode', 'terrain_conditions'],
  job_offers: ['id', 'job_id', 'operator_org_id', 'status', 'total_cents', 'price_cents', 'reliability_snapshot_json', 'offer_lines_json'],
  bookings: ['id', 'job_id', 'accepted_offer_id', 'buyer_org_id', 'executor_org_id', 'status', 'updated_at'],
  rate_cards: ['id', 'seller_org_id', 'service_type', 'is_active', 'created_at', 'updated_at'],
  orders: ['id', 'buyer_org_id', 'seller_org_id', 'status', 'order_status', 'payment_status', 'shipping_address', 'billing_address'],
  organizations: ['id', 'legal_name', 'is_certified', 'can_buy', 'can_sell', 'can_operate', 'can_dispatch'],
...
async function verifySchema() {
  try {
    ...
    console.log(`\n📊 Risultati tabelle:`);
    console.log(`   ✅ Esistenti: ${existingTables.length}/${expectedTables.length}`);
    ...
    for (const [table, columns] of Object.entries(criticalColumns)) {
      ...
      if (!tableExistsCheck) {
        console.log(`   ⚠️  ${table}: tabella non esiste, saltata verifica colonne`);
        continue;
      }
    ...
    console.log(`\n📊 Risultati colonne:`);
    console.log(`   ✅ Verificate: ${totalColumnsChecked - missingColumns}/${totalColumnsChecked}`);
    ...
    const idColumns = ['users.id', 'organizations.id', 'jobs.id', 'job_offers.id', 'bookings.id', 'shopping_carts.id', 'cart_items.id'];
    let uuidIssues = 0;
    ...
    for (const colRef of idColumns) {
      ...
      if (colInfo) {
        ...
        } else if (dataType.includes('varchar') || dataType.includes('character varying') || dataType === 'text') {
    ...
    console.log(`   Tabelle: ${existingTables.length}/${expectedTables.length} esistenti`);
    console.log(`   Colonne critiche: ${totalColumnsChecked - missingColumns}/${totalColumnsChecked} presenti`);
    console.log(`   Colonne ID UUID: ${uuidIssues} problemi`);
    
    if (missingTables.length === 0 && missingColumns === 0 && uuidIssues === 0) {
