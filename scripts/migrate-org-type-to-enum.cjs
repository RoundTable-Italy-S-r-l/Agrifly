...
async function migrateOrgTypeToEnum() {
  ...
  try {
    ...
    log('blue', `   Valori nell'enum: [${enumValues.map(v => `"${v}"`).join(', ')}]`);
    ...
    if (missingValues.length > 0) {
      log('yellow', `   ⚠️  Valori mancanti (lowercase): [${missingValues.join(', ')}]`);
    ...
    for (const org of valuesResult.rows) {
      ...
      if (!currentValue || currentValue === 'null' || currentValue === 'NULL') {
        ...
        log('yellow', `   🔧 ${orgId}: "${currentValue || 'NULL'}" → "${targetValue}" (dedotto)`);
      } else if (valueMapping[currentValue]) {
        targetValue = valueMapping[currentValue];
        log('yellow', `   🔧 ${orgId}: "${currentValue}" → "${targetValue}" (legacy)`);
      } else {
        // Normalizza case
        ...
        if (['buyer', 'vendor', 'operator'].includes(normalized)) {
          ...
          if (currentValue !== normalized) {
            log('yellow', `   🔧 ${orgId}: "${currentValue}" → "${targetValue}" (normalizzato)`);
          } else {
    ...
    finalCheckResult.rows.forEach(row => {
      ...
      const isValid = normalized && ['buyer', 'vendor', 'operator'].includes(normalized);
    ...
    log('blue', `   Tipo attuale: ${currentDataType} (${currentUdtName || 'N/A'})`);
    ...
    if (currentDataType === 'USER-DEFINED' && currentUdtName === 'OrgType') {
      ...
      try {
        ...
        log('blue', `      Usando "${vendorTarget}" per vendor (${hasVendorLowercase ? 'lowercase presente' : 'solo uppercase'})`);
        ...
        if (nullCount > 0) {
          log('red', `      ❌ ${nullCount} valori non convertiti. Rollback...`);
        ...
        log('yellow', '   💡 La colonna rimane TEXT. Puoi riprovare dopo aver corretto i problemi.');
        throw error;
      }
    }
    ...
    if (finalTypeResult.rows.length > 0) {
      ...
      if (finalDataType === 'USER-DEFINED' && finalUdtName === 'OrgType') {
        ...
        log('yellow', `⚠️  organizations.type è ancora ${finalDataType} (${finalUdtName})`);
      }
    }
