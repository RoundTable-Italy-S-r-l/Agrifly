...
async function syncSQLiteToSupabase() {
  ...
  try {
    ...
    if (usersWithoutOrgs.rows.length > 0) {
      ...
      for (const user of usersWithoutOrgs.rows) {
        await pgClient.query('DELETE FROM org_memberships WHERE user_id = $1', [user.id]);
      }
    ...
    const sqliteUsers = sqliteDb.prepare('SELECT id, email, first_name, last_name, role, email_verified, password_hash, password_salt FROM users').all();
    const pgUsers = await pgClient.query('SELECT id, email, first_name, last_name, role, email_verified FROM users');
    ...
    for (const sqliteUser of sqliteUsers) {
      ...
      if (!pgUser) {
        console.log(`  ⚠️  Utente ${sqliteUser.email} non trovato in Supabase (verrà aggiunto solo se necessario)`);
        continue;
      }
    ...
    const sqliteOrgs = sqliteDb.prepare(`
    ...
    for (const sqliteOrg of sqliteOrgs) {
      ...
      if (!pgOrg) {
        console.log(`  ⚠️  Organizzazione ${sqliteOrg.legal_name} non trovata in Supabase (verrà aggiunta solo se necessario)`);
        continue;
      }
    ...
    const finalUsersCount = await pgClient.query('SELECT COUNT(*) as count FROM users');
    const finalOrgsCount = await pgClient.query('SELECT COUNT(*) as count FROM organizations');
    
    console.log(`\n  Utenti totali: ${finalUsersCount.rows[0].count}`);
    console.log(`  Utenti senza organizzazioni: ${finalUsersWithoutOrgs.rows[0].count}`);
