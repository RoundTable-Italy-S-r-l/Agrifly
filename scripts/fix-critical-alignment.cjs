...
async function fixCriticalAlignment() {
  ...
  try {
    ...
    for (const value of orgTypeValues) {
      try {
        await client.query(`ALTER TYPE "OrgType" ADD VALUE IF NOT EXISTS '${value}';`);
        ...
        if (error.message.includes('IF NOT EXISTS')) {
          ...
          try {
            ...
            if (error2.message.includes('already exists') || error2.message.includes('duplicate')) {
              ...
              log('yellow', `   ⚠️  Errore aggiunta "${value}" a OrgType: ${error2.message}`);
            }
          }
        } else if (error.message.includes('already exists') || error.message.includes('duplicate')) {
          log('yellow', `   ℹ️  "${value}" già presente in OrgType`);
        } else {
          log('yellow', `   ⚠️  Errore aggiunta "${value}" a OrgType: ${error.message}`);
        }
      }
    }
    ...
    for (const value of bookingStatusValues) {
      try {
        await client.query(`ALTER TYPE "BookingStatus" ADD VALUE IF NOT EXISTS '${value}';`);
        log('green', `   ✅ Aggiunto "${value}" a BookingStatus`);
      } catch (error) {
        if (error.message.includes('IF NOT EXISTS')) {
          try {
            await client.query(`ALTER TYPE "BookingStatus" ADD VALUE '${value}';`);
            log('green', `   ✅ Aggiunto "${value}" a BookingStatus`);
          } catch (error2) {
            if (error2.message.includes('already exists') || error2.message.includes('duplicate')) {
              log('yellow', `   ℹ️  "${value}" già presente in BookingStatus`);
            } else {
              log('yellow', `   ⚠️  Errore aggiunta "${value}" a BookingStatus: ${error2.message}`);
            }
          }
        } else if (error.message.includes('already exists') || error.message.includes('duplicate')) {
          log('yellow', `   ℹ️  "${value}" già presente in BookingStatus`);
        } else {
          log('yellow', `   ⚠️  Errore aggiunta "${value}" a BookingStatus: ${error.message}`);
        }
      }
    }
    ...
    for (const { table, column } of idColumns) {
      ...
      try {
        ...
        if (!defaultValue) {
          ...
          `;
          const seqExists = await client.query(sequenceExistsQuery, [sequenceName]);
          ...
          if (dataType === 'text' || dataType === 'character varying') {
            ...
            try {
              ...
              `);
              log('green', `   ✅ ${table}.${column}: DEFAULT impostato (gen_random_uuid())`);
            } catch (error) {
              ...
              `);
              log('green', `   ✅ ${table}.${column}: DEFAULT impostato (sequence)`);
            }
          } else {
    ...
    log('yellow', '   1. Verifica che tutti i valori in organizations.type siano validi per OrgType');
    log('yellow', '   2. Crea una colonna temporanea');
    log('yellow', '   3. Migra i dati');
    log('yellow', '   4. Rinomina le colonne');
    log('yellow', '   Per ora il codice funziona perché PostgreSQL accetta TEXT per enum');
