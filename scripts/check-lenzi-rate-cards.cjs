...
async function checkLenziRateCards() {
  try {
    ...
    const orgColumn = columnNames.includes("seller_org_id")
      ? "seller_org_id"
      : columnNames.includes("org_id")
        ? "org_id"
        : null;

    if (!orgColumn) {
      console.log(
        "❌ Nessuna colonna org_id o seller_org_id trovata in rate_cards",
      );
      return;
    }
    ...
    const sqliteRateCards = sqliteDb
      .prepare(
        `
    ...
    if (sqliteRateCards.length > 0) {
      sqliteRateCards.forEach((card, index) => {
        ...
        console.log(
          `     - base_rate_per_ha_cents: ${card.base_rate_per_ha_cents || "N/A"}`,
        );
    ...
    if (pgRateCards.rows.length > 0) {
      pgRateCards.rows.forEach((card, index) => {
        console.log(`\n  ${index + 1}. Rate Card ID: ${card.id}`);
        console.log(
          `     - org_id/seller_org_id: ${card.org_id || card.seller_org_id}`,
        );
        console.log(`     - service_type: ${card.service_type || "N/A"}`);
        console.log(`     - crop_type: ${card.crop_type || "N/A"}`);
        console.log(`     - treatment_type: ${card.treatment_type || "N/A"}`);
