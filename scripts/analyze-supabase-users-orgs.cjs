...
async function analyzeSupabase() {
  ...
  try {
    ...
    console.log(
      "📊 PANORAMICA UTENTI, RUOLI, PERMESSI E ORGANIZZAZIONI SU SUPABASE",
    );
    console.log("=".repeat(100));
    console.log("");
    ...
    for (const [userId, user] of usersMap) {
      ...
      if (user.memberships.length === 0) {
        ...
        console.log(
          `     Membri di ${user.memberships.length} organizzazione/i:`,
        );
    ...
    if (inactiveMembers.rows.length > 0) {
      ...
      for (const mem of inactiveMembers.rows) {
        console.log(
          `     - ${mem.email} in ${mem.legal_name} (ruolo: ${mem.role})`,
        );
      }
    }
