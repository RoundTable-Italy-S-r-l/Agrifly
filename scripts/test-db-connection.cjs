...
async function testConnection() {
  try {
    ...
    const tables = await client.query(`
      ...
      ORDER BY table_name
    `);
    console.log('Tabelle trovate:', tables.rows.map(r => r.table_name).join(', '));
    ...
    `);
    const requiredCols = ['id', 'name', 'brand', 'model', 'product_type', 'status', 'specs_json', 'images_json', 'glb_files_json', 'specs_core_json'];
    const existingCols = productCols.rows.map(c => c.column_name);
    console.log('Colonne richieste:', requiredCols.join(', '));
    console.log('Colonne mancanti:', requiredCols.filter(c => !existingCols.includes(c)).join(', ') || 'Nessuna');
    ...
    if (assetsCheck.rows.length === 0) {
      ...
      console.log('  - Colonne:', assetCols.rows.map(c => c.column_name).join(', '));
    }
