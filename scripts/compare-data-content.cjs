...
async function compareTableData(pgClient, sqliteDb, tableName) {
  try {
    ...
    try {
      const sqliteTableInfo = sqliteDb
  ...
  try {
    ...
    for (const tableName of tablesToCompare) {
      ...
      if (result.differentCount > 0) {
        console.log(
          `  ⚠️  Record con valori diversi: ${result.differentCount}`,
        );
        if (result.differentValues.length > 0) {
          ...
          example.differences.slice(0, 3).forEach((diff) => {
            console.log(
              `       - ${diff.field}: Supabase="${diff.supabase}" vs SQLite="${diff.sqlite}"`,
            );
          });
        }
        tablesWithDifferences++;
      ...
      if (
        result.onlyInSupabase === 0 &&
        result.onlyInSQLite === 0 &&
    ...
    const tablesWithData = Object.entries(results).filter(
      ([_, r]) => r.exists && !r.error,
    );
    const allSame = tablesWithData.filter(
      ([_, r]) =>
        r.onlyInSupabase === 0 &&
        r.onlyInSQLite === 0 &&
        r.differentCount === 0,
    ...
    console.log(
      `✅ Tabelle identiche: ${allSame.length}/${tablesWithData.length}`,
    );
      ...
      for (const [tableName, result] of Object.entries(results)) {
        if (result.exists && !result.error) {
          if (
            ...
            if (result.onlyInSupabase > 0)
              console.log(`    Solo in Supabase: ${result.onlyInSupabase}`);
            if (result.onlyInSQLite > 0)
              console.log(`    Solo in SQLite: ${result.onlyInSQLite}`);
            if (result.differentCount > 0)
              console.log(`    Valori diversi: ${result.differentCount}`);
          }
        }
      }
    }
