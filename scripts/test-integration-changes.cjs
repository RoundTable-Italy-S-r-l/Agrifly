...
if (fs.existsSync(indexPath)) {
  ...
  const checks = [
    {
      name: "Import saveCurrentPathAsRedirect",
      pattern: /import.*saveCurrentPathAsRedirect.*from.*auth-redirect/,
    },
    {
      name: "Uso saveCurrentPathAsRedirect in handleLoginClick",
      pattern: /saveCurrentPathAsRedirect\(\)/,
    },
...
const buyerLayoutPath = path.join(
  projectRoot,
  "client/components/BuyerLayout.tsx",
);
...
if (fs.existsSync(loginPath)) {
  ...
  const checks = [
    {
      name: "Import handlePostAuthRedirect",
      pattern: /import.*handlePostAuthRedirect.*from.*auth-redirect/,
    },
    {
      name: "Import saveCurrentPathAsRedirect",
      pattern: /import.*saveCurrentPathAsRedirect.*from.*auth-redirect/,
    ...
    {
      name: "Uso handlePostAuthRedirect in handleRegister",
      pattern: /handlePostAuthRedirect\(/,
    },
    {
      name: "Uso handlePostAuthRedirect in handleLogin",
      pattern: /handlePostAuthRedirect\(/,
    },
    {
      name: "Uso saveCurrentPathAsRedirect in useEffect",
      pattern: /saveCurrentPathAsRedirect\(\)/,
    },
  ];
  ...
  if (duplicateCount === 0) {
    ...
    console.log(
      `  ⚠️  Possibile logica duplicata (${duplicateCount} pattern trovati più volte)`,
    );
    warnings++;
  }
} else {
...
if (fs.existsSync(verifyEmailPath)) {
  ...
  const checks = [
    ...
    {
      name: "Import migrateGuestCart",
      pattern: /import.*migrateGuestCart.*from.*auth-redirect/,
    },
  ...
  // Verifica che non ci sia logica duplicata
  if (
    content.includes("navigate('/buyer')") &&
    content.includes("handlePostAuthRedirect")
  ) {
    console.log(
      "  ⚠️  Possibile logica duplicata (navigate diretto + handlePostAuthRedirect)",
    );
    warnings++;
  } else {
    console.log("  ✅ Nessuna logica duplicata trovata");
...
if (fs.existsSync(dashboardPath)) {
  ...
  const checks = [
    {
      name: "Import getDashboardPath",
      pattern: /import.*getDashboardPath.*from.*auth-redirect/,
    },
...
const requireAuthPath = path.join(
  projectRoot,
  "client/components/RequireAuth.tsx",
);
if (fs.existsSync(requireAuthPath)) {
  ...
  const checks = [
    ...
    {
      name: "Uso saveCurrentPathAsRedirect",
      pattern: /saveCurrentPathAsRedirect\(\)/,
    },
  ];
