...
function mapLegacyRole(legacyRole, orgType) {
  ...
  // Default
  console.warn(`⚠️  Ruolo non mappato: "${legacyRole}", usando default "admin"`);
  return 'admin';
}
  ...
  try {
    ...
    rolesQuery.rows.forEach(row => {
      ...
      if (!roleStats[legacy].newRole) {
        roleStats[legacy].newRole = mapLegacyRole(row.legacy_role, row.org_type);
      }
    });
    
    console.log('\n📊 Statistiche ruoli legacy:');
    Object.entries(roleStats).forEach(([legacy, stats]) => {
      console.log(`   ${legacy || 'NULL'}: ${stats.count} membri → ${stats.newRole}`);
    });
    ...
    const missingRoles = newRoles.filter(r => !existingEnumValues.includes(r));
    ...
    if (missingRoles.length > 0) {
      ...
      for (const role of missingRoles) {
        try {
          await client.query(`ALTER TYPE "OrgRole" ADD VALUE IF NOT EXISTS '${role}'`);
    ...
    for (const row of rolesQuery.rows) {
      ...
      try {
        ...
        if (updated <= 10 || updated % 100 === 0) {
          console.log(`   ✅ [${updated}] ${row.id}: ${legacyRole || 'NULL'} → ${newRole}`);
        }
      } catch (err) {
    ...
    const invalidRoles = finalQuery.rows
      .filter(r => !['admin', 'vendor', 'operator', 'dispatcher'].includes(r.role))
      .map(r => r.role);
