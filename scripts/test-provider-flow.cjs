...
async function testProviderFlow() {
  ...
  try {
    ...
    const orgRoleValues = await client.query(`
      ...
      ORDER BY value
    `);
    const hasVendor = orgRoleValues.rows.some(r => r.value === 'VENDOR' || r.value === 'vendor');
    const hasOperator = orgRoleValues.rows.some(r => r.value === 'OPERATOR' || r.value === 'operator');
    ...
    const providerCount = orgStats.rows.find(r => r.type === 'provider')?.count || 0;
    if (parseInt(providerCount) < 5) {
      console.log(`  ⚠️  Attenzione: solo ${providerCount} organizzazioni provider (attese almeno 5)`);
    } else {
      console.log(`  ✅ ${providerCount} organizzazioni provider migrate correttamente`);
    }
    ...
    if (orgTypeStats.rows.length > 0) {
      ...
      const legacyOrgType = orgTypeStats.rows.filter(r => 
        r.org_type === 'vendor' || r.org_type === 'operator' || r.org_type === 'VENDOR'
      );
    ...
    console.log('  3. Verificare che i ruoli vendor/operator funzionino correttamente');
