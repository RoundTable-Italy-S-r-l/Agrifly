...
async function checkLenziServices() {
  try {
        ...
        columns.forEach((col) => {
          if (!["id", "org_id", "created_at", "updated_at"].includes(col)) {
            ...
            if (value !== null && value !== undefined && value !== "") {
              console.log(
                `       * ${col}: ${typeof value === "object" ? JSON.stringify(value).substring(0, 100) : value}`,
              );
            }
          }
        });
    ...
    const tableInfo = sqliteDb
      .prepare(`PRAGMA table_info(service_configurations)`)
      .all();
