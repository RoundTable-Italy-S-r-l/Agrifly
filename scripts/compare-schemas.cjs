...
async function compareSchemas() {
  try {
    ...
    const onlyInSupabase = supabaseTableNames.filter(t => !sqliteTableNames.includes(t));
    ...
    const onlyInSQLite = sqliteTableNames.filter(t => !supabaseTableNames.includes(t));
    ...
    // Tabelle comuni
    const commonTables = supabaseTableNames.filter(t => sqliteTableNames.includes(t));
    ...
    `);
    const hasIsCertifiedSupabase = supabaseOrgColumns.rows.some(c => c.column_name === 'is_certified');
    console.log(`Supabase: ${hasIsCertifiedSupabase ? '✅ Presente' : '❌ MANCANTE'}`);

    // SQLite
    const sqliteOrgColumns = sqliteDb.prepare(`PRAGMA table_info(organizations)`).all();
    const hasIsCertifiedSQLite = sqliteOrgColumns.some(c => c.name === 'is_certified');
    console.log(`SQLite locale: ${hasIsCertifiedSQLite ? '✅ Presente' : '❌ Mancante'}\n`);

    if (!hasIsCertifiedSupabase) {
      console.log('⚠️  La colonna is_certified NON esiste in Supabase!');
      console.log('   Deve essere aggiunta per far funzionare la feature dei preventivi certificati.\n');
    }

    // Confronto colonne organizations
    console.log('📋 CONFRONTO COLONNE organizations:\n');
    
    const supabaseCols = supabaseOrgColumns.rows.map(c => c.column_name).sort();
    const sqliteCols = sqliteOrgColumns.map(c => c.name).sort();
    
    const onlyInSupabaseCols = supabaseCols.filter(c => !sqliteCols.includes(c));
    const onlyInSQLiteCols = sqliteCols.filter(c => !supabaseCols.includes(c));
