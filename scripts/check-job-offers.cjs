...
async function checkJobOffers() {
  ...
  try {
    ...
    const orgResult = await client.query('SELECT id, legal_name, type FROM organizations WHERE id = $1', [orgId]);
    console.log(`   Organizzazioni trovate: ${orgResult.rows.length}`);
    if (orgResult.rows.length > 0) {
      console.log(`   - ${orgResult.rows[0].legal_name} (type: ${orgResult.rows[0].type})`);
    }
    console.log('');

    // 2. Conta tutte le job_offers
    console.log('2️⃣ Conta tutte le job_offers nel database...');
    const allOffersResult = await client.query('SELECT COUNT(*) as count FROM job_offers');
    ...
    console.log('3️⃣ Conta job_offers dove operator_org_id = lenzi-org-id (offers "made")...');
    ...
    if (madeOffersResult.rows[0].count > 0) {
      ...
      madeOffersDetail.rows.forEach((offer, i) => {
        console.log(`   ${i + 1}. ID: ${offer.id}, job_id: ${offer.job_id}, status: ${offer.status}, total_cents: ${offer.total_cents}, created: ${offer.created_at}`);
      });
    }
    console.log('');
    ...
    console.log(`   Jobs dove lenzi-org-id è buyer: ${buyerJobsResult.rows[0].count}`);
    ...
    if (buyerJobsResult.rows[0].count > 0) {
      ...
      buyerJobsDetail.rows.forEach((job, i) => {
        console.log(`   ${i + 1}. ID: ${job.id}, field_name: ${job.field_name}, status: ${job.status}, created: ${job.created_at}`);
      });
    }
    console.log('');

    // 5. Conta job_offers per jobs dove buyer_org_id = lenzi-org-id (offers "received")
    console.log('5️⃣ Conta job_offers per jobs dove buyer_org_id = lenzi-org-id (offers "received")...');
    const receivedOffersResult = await client.query(`
      ...
      WHERE j.buyer_org_id = $1
    `, [orgId]);
    console.log(`   Job offers "received": ${receivedOffersResult.rows[0].count}`);
    ...
    if (receivedOffersResult.rows[0].count > 0) {
      ...
      receivedOffersDetail.rows.forEach((offer, i) => {
        console.log(`   ${i + 1}. ID: ${offer.id}, job_id: ${offer.job_id}, field_name: ${offer.field_name}, operator_org_id: ${offer.operator_org_id}, status: ${offer.status}, created: ${offer.created_at}`);
      });
    }
    console.log('');
    ...
    const receivedTestResult = await client.query(`
      ...
      ORDER BY jo.created_at DESC
    `, [orgId]);
    console.log(`   Risultati query "received": ${receivedTestResult.rows.length}`);
    if (receivedTestResult.rows.length > 0) {
      console.log('   Prima offerta:');
      const first = receivedTestResult.rows[0];
      console.log(`   - ID: ${first.id}, job_id: ${first.job_id}, field_name: ${first.field_name}, status: ${first.status}`);
    }
    console.log('');
    ...
    allOffersDetail.rows.forEach((offer, i) => {
      console.log(`   ${i + 1}. ID: ${offer.id}, job_id: ${offer.job_id}, operator_org_id: ${offer.operator_org_id}, buyer_org_id: ${offer.buyer_org_id || 'NULL'}, status: ${offer.status}`);
    });
    console.log('');
    ...
    otherOrgsResult.rows.forEach((row, i) => {
      console.log(`   ${i + 1}. operator_org_id: ${row.operator_org_id || 'NULL'}, count: ${row.count}`);
    });
    console.log('');
