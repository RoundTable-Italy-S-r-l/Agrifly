  ...
  try {
    ...
    if (!ORG_ID) {
      ...
      orgs.rows.forEach(org => {
        console.log(`      - ${org.id}: ${org.legal_name} (can_operate: ${org.can_operate}, status: ${org.status})`);
      });
    ...
    console.log(`📋 Configurazione organizzazione: ${org.legal_name} (${ORG_ID})\n`);
    ...
    if (serviceConfigCheck.rows.length === 0) {
      ...
      console.log(`   ✅ Service configuration creata (lat: ${BASE_LAT}, lng: ${BASE_LNG})\n`);
    } else {
      await client.query(`
      ...
      console.log(`   ✅ Service configuration aggiornata (lat: ${BASE_LAT}, lng: ${BASE_LNG})\n`);
    }
    ...
    if (finalCheck.rows.length > 0) {
      ...
      console.log(`   ✅ Base Location: ${result.base_location_lat || 'N/A'}, ${result.base_location_lng || 'N/A'}`);
