  ...
  try {
    ...
    if (userCheck.rows.length === 0) {
      ...
      userCheck.rows.forEach((row, idx) => {
        ...
        console.log(`     Membership Active: ${row.membership_active || 'NULL'}`);
        console.log(`     Org ID: ${row.org_id || 'NULL'}`);
        console.log(`     Org Name: ${row.legal_name || 'NULL'}`);
        console.log(`     Org Type: ${row.org_type || row.org_type_legacy || 'NULL'}`);
        console.log('');
      });
    }
    ...
    memberships.rows.forEach(m => {
      console.log(`   - ${m.email} (${m.user_id}) - Role: ${m.role || 'NULL'} - User Status: ${m.user_status}`);
    });
    console.log('');
    ...
    rateCards.rows.forEach(rc => {
      console.log(`   - ${rc.service_type} (ID: ${rc.id}) - Active: ${rc.is_active} - Base Rate: ${rc.base_rate_per_ha_cents} centesimi`);
    });
    console.log('');
    ...
    console.log(`   Valori enum trovati: ${enumValues.rows.map(r => r.role_value).join(', ')}`);
    console.log('');
    ...
    actualRoles.rows.forEach(r => {
      console.log(`   - ${r.role_value}: ${r.count} membership (org types: ${r.org_types})`);
    });
    console.log('');

    // 6c. Controlla ruolo specifico per giacomo.cavalcabo14@gmail.com e lenzi-org-id
    console.log('🔍 6c. Ruolo esatto per giacomo.cavalcabo14@gmail.com in lenzi-org-id:');
    ...
    if (userRoleCheck.rows.length > 0) {
      ...
      console.log(`   ⚠️  PROBLEMA POTENZIALE: Il ruolo nel DB è in MAIUSCOLO, ma il codice potrebbe aspettarsi minuscolo!`);
    } else {
    ...
    if (userColumns.rows.length === 0) {
      ...
      console.log(`   ✅ Colonna "role" esiste: ${userColumns.rows[0].data_type}, nullable: ${userColumns.rows[0].is_nullable}`);
    }
    console.log('');
    ...
    console.log('🔍 9. Membership duplicate o problematiche per giacomo.cavalcabo14@gmail.com:');
    ...
    console.log(`   Trovate ${duplicateMemberships.rows.length} membership totali`);
    duplicateMemberships.rows.forEach((m, idx) => {
      console.log(`   ${idx + 1}. Org: ${m.legal_name} (${m.org_id}) - Role: ${m.role || 'NULL'} - Active: ${m.is_active} - Created: ${m.created_at}`);
    });
