...
async function compareUsersAndOrgs() {
  ...
  try {
    ...
    const sqliteUsers = sqliteDb.prepare('SELECT id, email, first_name, last_name, role, email_verified, created_at FROM users ORDER BY created_at DESC').all();
    ...
    for (const userId of commonUserIds) {
      ...
      if (pgUser.email !== sqliteUser.email) differences.push({ field: 'email', pg: pgUser.email, sqlite: sqliteUser.email });
      if (pgUser.first_name !== sqliteUser.first_name) differences.push({ field: 'first_name', pg: pgUser.first_name, sqlite: sqliteUser.first_name });
      if (pgUser.last_name !== sqliteUser.last_name) differences.push({ field: 'last_name', pg: pgUser.last_name, sqlite: sqliteUser.last_name });
      if (pgUser.role !== sqliteUser.role) differences.push({ field: 'role', pg: pgUser.role, sqlite: sqliteUser.role });
      if (pgUser.email_verified !== sqliteUser.email_verified) differences.push({ field: 'email_verified', pg: pgUser.email_verified, sqlite: sqliteUser.email_verified });
      
      if (differences.length > 0) {
        differentUsers.push({ id: userId, email: sqliteUser.email, differences });
      }
    }
    ...
    if (differentUsers.length > 0) {
      ...
      for (const user of differentUsers.slice(0, 5)) {
        ...
        for (const diff of user.differences) {
          console.log(`    ${diff.field}: Supabase="${diff.pg}" vs SQLite="${diff.sqlite}"`);
        }
      }
    ...
    const sqliteOrgs = sqliteDb.prepare('SELECT id, legal_name, COALESCE(type, org_type, \'buyer\') as org_type, status, is_certified, created_at FROM organizations ORDER BY created_at DESC').all();
    ...
    if (onlyInSQLiteOrgs.length > 0) {
      console.log(`⚠️  Organizzazioni solo in SQLite (${onlyInSQLiteOrgs.length}):`);
    ...
    if (onlyInPGOrgs.length > 0) {
      console.log(`ℹ️  Organizzazioni solo in Supabase (${onlyInPGOrgs.length}):`);
    ...
    for (const orgId of commonOrgIds) {
      ...
      // Compare fields
      if (pgOrg.legal_name !== sqliteOrg.legal_name) differences.push({ field: 'legal_name', pg: pgOrg.legal_name, sqlite: sqliteOrg.legal_name });
      if (pgOrg.org_type !== sqliteOrg.org_type) differences.push({ field: 'org_type', pg: pgOrg.org_type, sqlite: sqliteOrg.org_type });
      if (pgOrg.status !== sqliteOrg.status) differences.push({ field: 'status', pg: pgOrg.status, sqlite: sqliteOrg.status });
      if (pgOrg.is_certified !== sqliteOrg.is_certified) differences.push({ field: 'is_certified', pg: pgOrg.is_certified, sqlite: sqliteOrg.is_certified });
      
      if (differences.length > 0) {
        differentOrgs.push({ id: orgId, legal_name: sqliteOrg.legal_name, differences });
      }
    }
    
    if (differentOrgs.length > 0) {
      console.log(`⚠️  Organizzazioni con valori diversi (${differentOrgs.length}):`);
    ...
    console.log(`⚠️  Organizzazioni da aggiungere a Supabase: ${onlyInSQLiteOrgs.length}`);
    console.log(`⚠️  Utenti con valori diversi: ${differentUsers.length}`);
    console.log(`⚠️  Organizzazioni con valori diversi: ${differentOrgs.length}`);
    console.log('');
