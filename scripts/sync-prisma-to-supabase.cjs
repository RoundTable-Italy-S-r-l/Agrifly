...
const tableDefinitions = {
  ...
  rate_cards: {
    ...
    hilly_terrain_surcharge_cents: { type: 'INTEGER', default: '0', nullable: true },
...
async function createTable(tableName, columns) {
  ...
  for (const constraint of uniqueConstraints) {
    try {
      await client.query(`ALTER TABLE ${tableName} ADD CONSTRAINT ${tableName}_${constraint.match(/\((\w+)\)/)[1]}_unique ${constraint}`);
    } catch (e) {
...
async function addColumn(tableName, columnName, columnDef) {
  ...
  try {
    ...
    if (e.message.includes('already exists')) {
      ...
      console.log(`   ⚠️  Errore aggiunta colonna ${columnName} a ${tableName}:`, e.message);
    }
  }
}
...
async function syncSchema() {
  try {
    ...
    console.log(`   Verificando ${Object.keys(tableDefinitions).length} tabelle...\n`);
