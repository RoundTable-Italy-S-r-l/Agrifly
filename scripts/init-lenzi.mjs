...
async function initializeLenziCatalogStandalone() {
  ...
  console.log(`🔌 Connessione database: ${isSQLite ? 'SQLite locale' : 'PostgreSQL remoto'}...`);
  ...
  try {
    ...
    if (isSQLite) {
      ...
      try {
        ...
        const result = execSync(checkTables, { encoding: 'utf8', stdio: 'pipe' });
        ...
        console.log('⚠️  Errore nell\'inizializzazione SQLite, procedo comunque...');
      }
    } else {
      // PostgreSQL
    ...
    try {
      const lenziResult = execSync(lenziCheck, { encoding: 'utf8', stdio: 'pipe' });
      if (!lenziResult.includes('lenzi-org-id')) {
        throw new Error('❌ Organizzazione Lenzi (lenzi-org-id) non trovata nel database');
      }
    ...
    try {
      const countResult = execSync(countCmd, { encoding: 'utf8', stdio: 'pipe' });
      const productCount = countResult.match(/(\d+)/)?.[1] || '0';
      console.log(`📦 Trovati ${productCount} prodotti attivi nel database`);

      if (productCount === '0') {
        console.log('⚠️  Nessun prodotto trovato. Creo alcuni prodotti di esempio...');
      ...
      console.log('⚠️  Errore nel contare/creare prodotti, procedo comunque...');
    }
    ...
    const matches = lastLine.match(/(\d+)\s*\|\s*(\d+)\s*\|\s*(\d+)\s*\|\s*(\d+)/);
