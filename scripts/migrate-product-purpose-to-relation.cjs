...
(async () => {
  ...
  try {
    ...
    const products = await client.query('SELECT id, name, purpose FROM products WHERE purpose IS NOT NULL');
    ...
    for (const product of products.rows) {
      try {
        ...
        console.log(`    ❌ Errore parsing purpose per ${product.name}:`, error.message);
      }
    }
    ...
    console.log('💡 Nota: La colonna purpose vecchia è ancora presente. Puoi rimuoverla manualmente dopo verifica.');
  } catch (error) {
