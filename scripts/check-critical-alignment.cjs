  ...
  try {
    ...
    for (const [enumName, config] of Object.entries(criticalEnums)) {
      ...
      const missing = codeValues.filter((codeVal) => {
        if (caseSensitive) {
          ...
          return !dbValues.some(
            (dbVal) => dbVal.toLowerCase() === codeValLower,
          );
        }
      });
      ...
      if (missing.length > 0) {
        log(
          "red",
          `   ❌ ${enumName}: valori mancanti nel DB: [${missing.map((v) => `"${v}"`).join(", ")}]`,
        );
        log(
          "blue",
          `      Valori nel DB: [${dbValues.map((v) => `"${v}"`).join(", ")}]`,
        );
        criticalIssues.push({
    ...
    for (const {
      ...
      try {
        ...
        if (expectedType === "USER-DEFINED" && actualType !== "USER-DEFINED") {
          log(
            "red",
            `   ❌ ${table}.${column}: atteso enum (${expectedUdt}), trovato ${actualType}`,
          );
          ...
          log(
            "yellow",
            `   ⚠️  ${table}.${column}: enum diverso (atteso ${expectedUdt}, trovato ${actualUdt})`,
          );
        } else {
          log(
            "green",
            `   ✅ ${table}.${column}: tipo corretto (${actualType}${actualUdt ? ` / ${actualUdt}` : ""})`,
          );
        }
      } catch (error) {
        log(
    ...
    for (const { table, column } of idColumns) {
      ...
      try {
        ...
        if (!defaultValue && isNullable === "NO") {
          log(
            "red",
            `   ❌ ${table}.${column}: manca DEFAULT (tipo: ${dataType}, nullable: NO)`,
          );
          log(
            "blue",
            `      Potrebbe causare errori INSERT se l'id non viene fornito`,
          );
          criticalIssues.push({
    ...
    if (criticalIssues.length === 0) {
      ...
      criticalIssues.forEach((issue, index) => {
        ...
        if (issue.type === "enum_value_missing") {
          ...
          log(
            "red",
            `   Valori mancanti: [${issue.values.map((v) => `"${v}"`).join(", ")}]`,
          );
          log(
            "blue",
            `   Valori nel DB: [${issue.dbValues.map((v) => `"${v}"`).join(", ")}]`,
          );
      ...
      log(
        "yellow",
        "   - Per enum mancanti: usa ALTER TYPE \"EnumName\" ADD VALUE 'value';",
      );
      log(
        "yellow",
        "   - Per colonne senza DEFAULT: aggiungi DEFAULT o genera ID nel codice",
      );
      log(
        "yellow",
        "   - Per tipi errati: considera migrazione a enum se appropriato",
      );
    }
  } catch (error) {
