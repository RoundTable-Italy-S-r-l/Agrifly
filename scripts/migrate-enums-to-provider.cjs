...
async function migrateEnums() {
  ...
  try {
    ...
    if (!hasProvider) {
      ...
      await client.query(`ALTER TYPE "OrgType" ADD VALUE IF NOT EXISTS 'provider'`);
    ...
    if (!hasVendor) {
      ...
      await client.query(`ALTER TYPE "OrgRole" ADD VALUE IF NOT EXISTS 'VENDOR'`);
    ...
    if (orgCount.rows.length > 0) {
      ...
      for (const row of orgCount.rows) {
        const updateOrg = await client.query(`
          ...
          WHERE type::text = $1
        `, [row.type]);
        console.log(`  ✅ Migrate ${updateOrg.rowCount} organizzazioni da "${row.type}"`);
      }
    } else {
    ...
    if (orgTypeCount.rows.length > 0) {
      console.log('\n📊 org_type da migrare:');
      orgTypeCount.rows.forEach(r => console.log(`  ${r.org_type}: ${r.count}`));
      ...
      for (const row of orgTypeCount.rows) {
        ...
        `, [row.org_type]);
        console.log(`  ✅ Migrate ${updateOrgType.rowCount} organizzazioni (org_type) da "${row.org_type}"`);
      }
    }
