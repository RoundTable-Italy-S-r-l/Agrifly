...
async function verifySupabaseChanges() {
  ...
  try {
    ...
    const hasOperator = orgTypeValues.some(v => v.toLowerCase() === 'operator');
    ...
    const bookingStatusValues = bookingStatusResult.rows.map(r => r.enumlabel);
    ...
    for (const { table, column } of idColumns) {
      ...
      try {
        ...
        if (defaultValue) {
          const isNew = defaultValue.includes('gen_random_uuid') || defaultValue.includes('_seq');
          const marker = isNew ? ' ✨ CORRETTO' : '';
          log('green', `   ✅ ${table}.${column}: DEFAULT presente${marker}`);
          log('blue', `      Tipo: ${dataType}, Default: ${defaultValue.substring(0, 60)}${defaultValue.length > 60 ? '...' : ''}`);
        } else {
    ...
    try {
      ...
      if (result.rows.length > 0) {
        ...
        if (dataType === 'USER-DEFINED' && udtName === 'OrgType') {
          ...
          log('yellow', `      Il codice funziona ma sarebbe meglio migrare a enum`);
        } else {
          log('yellow', `   ⚠️  organizations.type ha tipo inaspettato: ${dataType}`);
        }
      } else {
    ...
    log('green', '   - BookingStatus: valori "AWARDED", "OFFERED", "WITHDRAWN" aggiunti');
    ...
    log('yellow', '   - Questo funziona ma richiede migrazione manuale per type safety');
