...
async function fixColumn(tableName, columnName) {
  try {
    ...
    if (currentType === "uuid") {
      console.log(
        `   🔄 Modifica ${tableName}.${columnName} da UUID a VARCHAR(255)...`,
      );
      ...
      console.log(
        `   ✅ Colonna ${tableName}.${columnName} già di tipo TEXT/VARCHAR`,
      );
      return false;
    } else {
      console.log(
        `   ⚠️  Tipo colonna ${tableName}.${columnName} sconosciuto: ${currentType}`,
      );
      return false;
    }
  } catch (error) {
    console.error(
      `   ❌ Errore durante modifica ${tableName}.${columnName}:`,
      error.message,
    );
    return false;
  }
