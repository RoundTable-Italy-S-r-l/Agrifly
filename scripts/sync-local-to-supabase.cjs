...
async function syncLocalToSupabase() {
  try {
    ...
    const columnsToAdd = [
      ...
      { name: 'can_operate', type: 'BOOLEAN', default: 'false', nullable: false },
      { name: 'can_dispatch', type: 'BOOLEAN', default: 'false', nullable: false },
    ...
    try {
      ...
      if (err.message.includes('already exists')) {
        ...
        console.error('  ❌ Errore creando service_configurations:', err.message);
      }
    }
    ...
    try {
      ...
      if (err.message.includes('already exists')) {
        ...
        console.error('  ❌ Errore creando conversation_participants:', err.message);
      }
    }
    ...
    const supabaseOrgs = await supabaseClient.query('SELECT id FROM organizations');
    ...
    for (const org of localOrgs) {
      if (!existingOrgIds.has(org.id)) {
        try {
          ...
          `);
          const hasOrgType = orgColumns.rows.some(c => c.column_name === 'org_type');
          const hasKind = orgColumns.rows.some(c => c.column_name === 'kind');
          
          const orgTypeValue = hasOrgType ? (org.org_type || org.kind || 'BUSINESS') : null;
          ...
          const orgCols = [
            'id', 'legal_name', 'logo_url', 'phone', 'support_email', 'vat_number', 'tax_code',
            'address_line', 'city', 'province', 'region', 'postal_code', 'country', 'status', 'created_at',
            'can_buy', 'can_sell', 'can_operate', 'can_dispatch', kindCol, 'type', 'is_certified'
          ].filter(c => c !== '');
          
          const orgVals = [
            org.id, org.legal_name, org.logo_url || null, org.phone || null, org.support_email || null,
            org.vat_number || null, org.tax_code || null, org.address_line, org.city, org.province,
            org.region, org.postal_code || null, org.country || 'IT', org.status || 'ACTIVE',
            parseTimestamp(org.created_at),
            org.can_buy !== undefined ? (org.can_buy === 1 || org.can_buy === true) : true,
            org.can_sell !== undefined ? (org.can_sell === 1 || org.can_sell === true) : false,
            org.can_operate !== undefined ? (org.can_operate === 1 || org.can_operate === true) : false,
            org.can_dispatch !== undefined ? (org.can_dispatch === 1 || org.can_dispatch === true) : false,
            kindVal, org.type || null,
            org.is_certified !== undefined ? (org.is_certified === 1 || org.is_certified === true) : false
          ...
          console.error(`    ⚠️  Errore inserendo organization ${org.id}:`, err.message);
        }
      }
    }
    console.log(`    ✅ Aggiunte ${orgsAdded} organizzazioni (${localOrgs.length - orgsAdded} già esistenti)`);
    ...
    for (const user of localUsers) {
      if (!existingUserIds.has(user.id)) {
        try {
          ...
          `);
          const needsUpdatedAt = userColumns.rows.length > 0 && userColumns.rows[0].is_nullable === 'NO';
          ...
          `, [
            ...
            user.phone || null, user.password_hash, user.password_salt || null,
            user.email_verified || false, user.email_verified_at ? parseTimestamp(user.email_verified_at) : null,
            user.status || 'ACTIVE', parseTimestamp(user.created_at),
            ...(needsUpdatedAt ? [parseTimestamp(user.updated_at || user.created_at)] : [])
          ]);
          usersAdded++;
        } catch (err) {
          console.error(`    ⚠️  Errore inserendo user ${user.id}:`, err.message);
        }
      }
    }
    console.log(`    ✅ Aggiunti ${usersAdded} utenti (${localUsers.length - usersAdded} già esistenti)`);

    // Migra altre tabelle comuni (jobs, job_offers, rate_cards, etc.)
    const commonTables = ['jobs', 'job_offers', 'rate_cards', 'products', 'skus'];
    ...
    try {
      ...
      `);
      console.log('    ✅ Tabella saved_fields creata/aggiornata con struttura corretta');
      
      // Migra i dati dal locale
      const localFields = sqliteDb.prepare('SELECT * FROM saved_fields').all();
      if (localFields.length > 0) {
        const existingFields = await supabaseClient.query('SELECT id FROM saved_fields');
        ...
        for (const field of localFields) {
          if (!existingIds.has(field.id)) {
            try {
              ...
              console.error(`    ⚠️  Errore inserendo saved_field ${field.id}:`, err.message.substring(0, 100));
            }
          }
        }
        console.log(`    ✅ saved_fields: aggiunti ${fieldsAdded} record (${localFields.length - fieldsAdded} già esistenti)`);
      } else {
    ...
    for (const tableName of commonTables) {
      try {
        ...
        if (supabaseColumns.rows.length === 0) {
          console.log(`    ⏭️  Tabella ${tableName} non esiste in Supabase, saltata`);
          continue;
        }
        ...
        const existingIds = await supabaseClient.query(`SELECT ${idColumn} FROM ${tableName}`);
          ...
          if (!existingIdSet.has(row[idColumn])) {
            try {
              ...
              const values = cols.map(c => {
                ...
                if (c.includes('_at') || c.includes('_date') || c.includes('created') || c.includes('updated') || 
                    c.includes('_start') || c.includes('_end') || c === 'proposed_start' || c === 'proposed_end') {
                  return parseTimestamp(val);
                }
                // Se è boolean e viene da SQLite (0/1)
                if (typeof val === 'number' && (val === 0 || val === 1)) {
                  const colInfo = supabaseColumns.rows.find(col => col.column_name === c);
                  if (colInfo && (colInfo.data_type === 'boolean' || colInfo.data_type === 'USER-DEFINED')) {
                    return val === 1;
                  }
                }
              ...
              if (!err.message.includes('violates foreign key') && 
                ...
                console.error(`    ⚠️  Errore inserendo in ${tableName} (ID: ${row[idColumn]}):`, err.message.substring(0, 100));
              }
            }
          }
        }
        ...
        if (added > 0) {
          console.log(`    ✅ ${tableName}: aggiunti ${added} record (${localData.length - added} già esistenti)`);
        }
      } catch (err) {
    ...
    try {
      const localConfigs = sqliteDb.prepare('SELECT * FROM service_configurations').all();
      if (localConfigs.length > 0) {
        const existing = await supabaseClient.query('SELECT id FROM service_configurations');
        ...
        for (const config of localConfigs) {
          if (!existingIds.has(config.id)) {
            try {
              ...
              `, [
                config.id, config.org_id, config.base_location_lat || null, config.base_location_lng || null,
                config.base_location_address || null, config.working_hours_start || 8, config.working_hours_end || 18,
                config.available_days || 'MON,TUE,WED,THU,FRI', config.offer_message_template || null,
                config.rejection_message_template || null, config.available_drones || null,
                config.preferred_terrain || null, config.max_slope_percentage || null,
                config.fuel_surcharge_cents || 0, config.maintenance_surcharge_cents || 0,
                config.enable_offer_filters || false, config.max_distance_from_base || null,
                config.accepted_service_types || null, config.min_price_per_ha_cents || null,
                config.max_price_per_ha_cents || null, config.accepted_terrain_conditions || null,
                config.max_accepted_slope || null, parseTimestamp(config.created_at),
                parseTimestamp(config.updated_at || config.created_at)
              ]);
              added++;
            } catch (err) {
              console.error(`    ⚠️  Errore inserendo service_configuration ${config.id}:`, err.message);
            }
          }
        }
      ...
      console.log(`    ⏭️  service_configurations non presente in locale o errore:`, err.message);
    }

    console.log('\n✅ Sincronizzazione completata!');
    console.log('\n📊 Riepilogo finale:');
    console.log('  ✅ Colonne aggiunte in organizations (can_buy, can_sell, can_operate, can_dispatch, kind, type)');
    console.log('  ✅ Tabelle create: service_configurations, conversations, conversation_participants, messages, job_offer_messages, order_messages');
    console.log('  ✅ saved_fields: struttura aggiornata e dati migrati');
    console.log('  ✅ Dati migrati da SQLite locale a Supabase (evitando duplicati)');
    console.log('\n💡 Prossimi passi:');
    console.log('  - Verifica i dati in Supabase');
    console.log('  - Se necessario, esegui di nuovo lo script per aggiornare i dati');
