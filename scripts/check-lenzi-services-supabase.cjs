...
async function checkLenziServices() {
  try {
    ...
    console.log(`📊 Service configurations trovate: ${serviceConfigs.rows.length}`);
    if (serviceConfigs.rows.length > 0) {
      serviceConfigs.rows.forEach((config, index) => {
        ...
        console.log(`     - work_start_hour: ${config.work_start_hour || config.working_hours_start || 'N/A'}`);
        console.log(`     - work_end_hour: ${config.work_end_hour || config.working_hours_end || 'N/A'}`);
    ...
    if (rateCards.rows.length > 0) {
      rateCards.rows.forEach((card, index) => {
        ...
        console.log(`     - base_rate_per_ha_cents: ${card.base_rate_per_ha_cents}`);
    ...
    console.log(`📊 Operator profiles trovati: ${operatorProfiles.rows.length}`);
    if (operatorProfiles.rows.length > 0) {
      operatorProfiles.rows.forEach((profile, index) => {
        ...
        console.log(`     - certifications_json: ${profile.certifications_json ? 'presente' : 'N/A'}`);
      });
    } else {
    ...
    if (serviceConfigs.rows.length === 0 && rateCards.rows.length === 0) {
      ...
      console.log(`   Potrebbero essere necessarie per funzionare correttamente.`);
    }
