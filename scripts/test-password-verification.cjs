...
async function testPassword() {
  try {
    ...
    console.log('  - Password hash (first 20):', user.password_hash?.substring(0, 20) || 'N/A');
    console.log('  - Password salt (first 20):', user.password_salt?.substring(0, 20) || 'N/A');
    ...
    const isValid = verifyPassword(testPassword, user.password_hash, user.password_salt);
    console.log('  - Risultato verifica:', isValid ? '✅ VALIDA' : '❌ NON VALIDA');
    
    if (!isValid) {
      console.log('\n⚠️  Password non corrisponde!');
      console.log('  - Hash memorizzato:', user.password_hash.substring(0, 40) + '...');
      ...
      const testHash = pbkdf2Sync(testPassword, testSaltBytes, 100000, 64, 'sha256').toString('hex');
      ...
      console.log('  - Verifica nuovo hash:', testVerify ? '✅ OK' : '❌ ERRORE');
    }
