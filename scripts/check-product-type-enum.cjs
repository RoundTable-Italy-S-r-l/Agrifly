...
async function checkProductTypeEnum() {
  ...
  try {
    ...
    const hasDroneLowercase = result.rows.some(
      (row) => row.enumlabel === "drone",
    );
    const hasDroneUppercase = result.rows.some(
      (row) => row.enumlabel === "DRONE",
    );
    ...
    console.log(
      `   lowercase ("drone"): ${hasDroneLowercase ? "✅ Esiste" : "❌ Non esiste"}`,
    );
    console.log(
      `   UPPERCASE ("DRONE"): ${hasDroneUppercase ? "✅ Esiste" : "❌ Non esiste"}`,
    );
    console.log(
      `   Mixed ("Drone"): ${hasDroneMixed ? "✅ Esiste" : "❌ Non esiste"}`,
    );

    if (!hasDroneLowercase && !hasDroneUppercase && !hasDroneMixed) {
      console.log('\n⚠️  Nessun valore "drone" trovato nell\'enum');
      console.log(
        '   Il codice cerca "drone" (lowercase) ma potrebbe non esistere',
      );
