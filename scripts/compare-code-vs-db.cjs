...
async function compareSchemas() {
  ...
  try {
      ...
      if (missingColumns.length > 0 || extraColumns.length > 0) {
        ...
        if (missingColumns.length > 0) {
          ...
          missingColumns.forEach((col) => {
            const colInfo = columnsResult.rows.find(
              (r) => r.column_name === col,
            );
            console.log(`      - ${col}`);
          });
    ...
    console.log(
      `❌ Tabelle mancanti: ${allDiscrepancies.missingTables.length}`,
    );
    console.log(`⚠️  Tabelle extra: ${allDiscrepancies.extraTables.length}`);
    console.log(
      `❌ Tabelle con colonne mancanti: ${Object.keys(allDiscrepancies.missingColumns).length}`,
    );
    console.log(
      `⚠️  Tabelle con colonne extra: ${Object.keys(allDiscrepancies.extraColumns).length}`,
    );

    const totalMissingColumns = Object.values(
      allDiscrepancies.missingColumns,
