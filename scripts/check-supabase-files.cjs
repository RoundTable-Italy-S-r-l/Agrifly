...
async function checkSupabaseFiles() {
  ...
  try {
    ...
    console.log('\n=== VIDEOS/T25 ===');
    const { data: videosT25Files, error: videosT25Error } = await supabase.storage
      .from(bucketName)
      .list('videos/t25', { limit: 50 });
    ...
    console.log('\n=== IMAGES/T25 ===');
    const { data: imagesT25Files, error: imagesT25Error } = await supabase.storage
      .from(bucketName)
      .list('images/t25', { limit: 50 });
