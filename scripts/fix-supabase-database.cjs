...
async function fixSupabaseDatabase() {
  ...
  try {
    ...
    if (enumTypeResult.rows.length === 0) {
      ...
      const hasDroneLowercase = enumResult.rows.some(
        (row) => row.enumlabel === "drone",
      );
      const hasDroneUppercase = enumResult.rows.some(
        (row) => row.enumlabel === "DRONE",
      );
      ...
      if (!hasDroneLowercase) {
        ...
        console.log(
          `   ℹ️  Valore "DRONE" (uppercase) ${hasDroneUppercase ? "presente" : "non presente"}`,
        );
        console.log(
          '   ➕ Aggiungo "drone" (lowercase) all\'enum ProductType...',
        );
      ...
      if (columnInfo.rows.length === 0) {
        ...
        if (!col.column_default && col.is_identity !== "YES") {
          ...
          if (
            ...
            const maxIdQuery =
              "SELECT COALESCE(MAX(id), 0)::bigint as max_id FROM organization_invitations;";
    ...
    if (finalEnumTypeResult.rows.length > 0) {
      ...
      const finalEnumQuery = `
        ...
        ORDER BY enumsortorder;
      `;
      const finalEnumResult = await client.query(finalEnumQuery, [
        finalEnumOid,
      ]);
      console.log(
        "   Enum ProductType:",
        finalEnumResult.rows.map((r) => `"${r.enumlabel}"`).join(", "),
      );
    }
