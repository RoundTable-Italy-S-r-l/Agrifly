  ...
  try {
    ...
    if (events.rows.length === 0) {
      ...
      events.rows.forEach((event, i) => {
        ...
        console.log(
          `    - Tempo risposta: ${event.response_seconds}s (${event.response_minutes.toFixed(2)} minuti)`,
        );
    ...
    if (metrics.rows.length === 0) {
      ...
      metrics.rows.forEach((metric, i) => {
        ...
        console.log(
          `    - Tempo medio: ${metric.avg_response_minutes.toFixed(2)} minuti`,
        );
        console.log(`    - Campione: ${metric.sample_count} risposte`);
        console.log(`    - Ultima risposta: ${metric.last_response_at}`);
        console.log(
          `    - Finestra calcolo: ${metric.calculation_window_days} giorni`,
        );
      });
    }
