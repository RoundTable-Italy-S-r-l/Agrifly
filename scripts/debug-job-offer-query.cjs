...
async function debugQuery() {
  ...
  try {
    ...
    const jobResult = await client.query("SELECT * FROM jobs WHERE id = $1", [
      jobId,
    ]);
    ...
    if (jobResult.rows.length > 0) {
      ...
      console.log(
        `   ❌ Job non trovato! Questo è il problema - il job_id nella job_offer non esiste nella tabella jobs.`,
      );
    }
    console.log("");
    ...
    joinResult.rows.forEach((row, i) => {
      console.log(
        `   ${i + 1}. offer_id: ${row.id}, job_id: ${row.job_id}, job_exists: ${row.job_exists || "NULL"}`,
      );
    });
    console.log("");
    ...
    buyerOrgResult.rows.forEach((row, i) => {
      console.log(
        `   ${i + 1}. offer_id: ${row.id}, job_id: ${row.job_id}, buyer_org_id: ${row.buyer_org_id}, buyer_org_exists: ${row.buyer_org_exists || "NULL"}`,
      );
    });
    console.log("");
    ...
    operatorOrgResult.rows.forEach((row, i) => {
      console.log(
        `   ${i + 1}. offer_id: ${row.id}, operator_org_id: ${row.operator_org_id}, operator_org_exists: ${row.operator_org_exists || "NULL"}`,
      );
    });
    console.log("");
    ...
    try {
      ...
      if (fullQueryResult.rows.length > 0) {
        ...
        console.log(
          `   Prima offerta: ID=${first.id}, field_name=${first.field_name}, status=${first.status}`,
        );
      }
    } catch (error) {
