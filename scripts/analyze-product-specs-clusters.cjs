      ...
      try {
        ...
        specs.forEach((spec) => {
          ...
          const normalizedKey = key
            .toLowerCase()
            .replace(
              /peso massimo al decollo per (spandimento|irrorazione|lo spandimento)/gi,
              "peso_massimo_decollo",
            )
            .replace(
              /raggio massimo di volo|massimo raggio di volo configurabile/gi,
              "raggio_max_volo",
            )
            .replace(
              /temperatura operativa (minima|massima)/gi,
              "temperatura_operativa",
            )
            .replace(
              /resistenza massima al vento|massima resistenza al vento/gi,
              "resistenza_vento",
            )
            .replace(
              /capacità serbatoio di (irrorazione|spandimento)/gi,
              "capacita_serbatoio",
            )
            .replace(
              /carico massimo per (irrorazione|spandimento)/gi,
              "carico_max",
            )
            .replace(
              /larghezza effettiva di (irrorazione|spandimento)|larghezza (dello spruzzo efficace|di spandimento effettiva)/gi,
              "larghezza_effettiva",
            )
            .replace(/peso batteria|peso/gi, "peso_batteria")
            .replace(/capacità batteria|capacità/gi, "capacita_batteria")
            .replace(
              /tempo di ricarica|tempo di ricarica\[6\]/gi,
              "tempo_ricarica",
            )
            .replace(
              /consumo carburante generatore|consumo di carburante di riferimento/gi,
              "consumo_carburante",
            )
            .replace(
              /portata rilevamento radar|portata\[4\]/gi,
              "portata_radar",
            )
            .replace(/[^\w]/g, "_")
            .replace(/_+/g, "_")
          ...
          if (allSpecs[section][normalizedKey].examples.length < 3) {
            allSpecs[section][normalizedKey].examples.push({
              key,
              value,
              unit,
        ...
        console.log(
          `⚠️  Errore parsing specs per ${product.name}:`,
          error.message,
        );
      }
    });
    ...
    Object.keys(allSpecs)
      ...
      .forEach((section) => {
        ...
        Object.entries(allSpecs[section]).forEach(([normalizedKey, data]) => {
          ...
          console.log(
            `    Esempi: ${data.examples.map((e) => `${e.product}: ${e.value} ${e.unit}`).join("; ")}`,
          );
        });
      });
