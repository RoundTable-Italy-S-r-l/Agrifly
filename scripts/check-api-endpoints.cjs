...
async function extractBackendRoutes() {
  ...
  const routeFiles = await glob("**/*-hono.ts", {
    cwd: serverDir,
    absolute: true,
  });
  ...
  const routeMounts =
    honoAppContent.match(/app\.route\(['"]\/api\/([^'"]+)['"]/g) || [];
...
function findMatchingBackendRoute(frontendEndpoint, backendRoutes) {
  ...
  const cleanEndpoint = frontendEndpoint.startsWith("/")
    ? frontendEndpoint.slice(1)
    : frontendEndpoint;

  // Cerca corrispondenze esatte o parziali
  for (const backendRoute of backendRoutes) {
    const cleanBackendRoute = backendRoute.startsWith("/")
      ? backendRoute.slice(1)
      : backendRoute;
...
async function main() {
  ...
  for (const frontendEndpoint of frontendEndpoints) {
    ...
    if (!match) {
      ...
      console.log(
        `⚠️  /api/${frontendEndpoint} - corrisponde a /api/${match} (potenziale discrepanza)`,
      );
    } else {
  ...
  if (warnings.length > 0) {
    ...
    warnings.forEach((w) =>
      console.log(
        `  Frontend: /api/${w.frontend} → Backend: /api/${w.backend}`,
      ),
    );
  }
