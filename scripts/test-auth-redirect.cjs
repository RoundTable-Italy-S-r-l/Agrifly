...
async function testAuthRedirect() {
  ...
  try {
    ...
    const expectedTables = ['users', 'organizations', 'org_memberships', 'response_events', 'response_metrics'];
    ...
    console.log('✅ Enum EntityType presente con valori:', enumCheck.rows.map(r => r.enumlabel).join(', '), '\n');
    ...
    const requiredColumns = [
      'id', 'conversation_id', 'requester_org_id', 'requester_user_id',
      'request_message_id', 'responder_org_id', 'responder_user_id',
      'response_message_id', 'response_seconds', 'response_minutes', 'created_at'
    ];
    ...
    const requiredMetricsColumns = [
      'id', 'entity_type', 'entity_id', 'avg_response_minutes',
      'sample_count', 'last_response_at', 'calculation_window_days', 'last_calculated_at'
    ];
    
    const foundMetricsColumns = metricsColumns.rows.map(r => r.column_name);
    const missingMetricsColumns = requiredMetricsColumns.filter(c => !foundMetricsColumns.includes(c));
    
    if (missingMetricsColumns.length > 0) {
      console.log('❌ Colonne mancanti in response_metrics:', missingMetricsColumns);
    } else {
      console.log('✅ Tutte le colonne necessarie sono presenti');
    }
    console.log('');
    ...
    console.log('\n📝 Note:');
    console.log('  - I test di funzionalità frontend (login, redirect, Index.tsx) richiedono test E2E');
    console.log('  - Verifica manuale:');
    console.log('    1. Login/registrazione e redirect alla pagina di origine');
    console.log('    2. Index.tsx mostra Login/Logout e Dashboard in base all\'auth');
    console.log('    3. "Vai al sito" nelle dashboard AdminLayout e BuyerLayout');
