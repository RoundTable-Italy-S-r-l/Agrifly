...
const schemas = require('./schemas/api.schemas.ts').default || require('./schemas/api.schemas.ts');
...
function testSchema(schemaName, schema, validData, invalidData = []) {
  ...
  invalidData.forEach((invalid, index) => {
    try {
      ...
      if (!result.success) {
        console.log(`  ✅ Invalid data ${index + 1}: PASSED (correctly rejected)`);
        testsPassed++;
      } else {
        console.log(`  ❌ Invalid data ${index + 1}: FAILED (should have been rejected)`);
        testsFailed++;
...
testSchema('LoginSchema', schemas.LoginSchema, {
...
testSchema('RegisterOrganizationSchema', schemas.RegisterOrganizationSchema, {
...
// 2. Schemi di job
testSchema('CreateJobSchema', schemas.CreateJobSchema, {
...
testSchema('CreateJobOfferSchema', schemas.CreateJobOfferSchema, {
  total_cents: 150000,
  currency: 'EUR',
...
// 3. Schemi di offerte
testSchema('CreateOfferSchema', schemas.CreateOfferSchema, {
  offer_type: 'BUNDLE',
...
testSchema('UpdateOfferSchema', schemas.UpdateOfferSchema, {
...
testSchema('CreateOrderFromCartSchema', schemas.CreateOrderFromCartSchema, {
  cart_items: [
    {
      sku_id: 'sku_test',
      quantity: 2,
...
testSchema('AddCartItemSchema', schemas.AddCartItemSchema, {
  sku_id: 'sku_t50',
  quantity: 1,
...
testSchema('CreateOperatorSchema', schemas.CreateOperatorSchema, {
  first_name: 'Mario',
  last_name: 'Rossi',
  ...
  { first_name: 'Mario', last_name: 'Rossi', email: 'mario@example.com', experience_years: -1 }, // Esperienza negativa
]);

// 6. Schemi di servizi
testSchema('CreateServiceSchema', schemas.CreateServiceSchema, {
  service_type: 'SPRAY',
...
testSchema('CreateAddressSchema', schemas.CreateAddressSchema, {
...
// 8. Schemi di inviti
testSchema('CreateInvitationSchema', schemas.CreateInvitationSchema, {
...
testSchema('CreateSavedFieldSchema', schemas.CreateSavedFieldSchema, {
  name: 'Campo Vigneti',
  area_ha: 25.5,
  crop_type: 'VINEYARD',
  location_json: { type: 'Point', coordinates: [9.1, 45.5] },
...
testSchema('CreateMessageSchema', schemas.CreateMessageSchema, {
...
// 11. Schemi di email
testSchema('VerifyEmailSchema', schemas.VerifyEmailSchema, {
  token: 'abc123def456'
}, [
  { token: '' }, // Token vuoto
...
testSchema('RequestPasswordResetSchema', schemas.RequestPasswordResetSchema, {
...
testSchema('ResetPasswordSchema', schemas.ResetPasswordSchema, {
  token: 'reset123token',
...
testSchema('AcceptOfferParamsSchema', schemas.AcceptOfferParamsSchema, {
  jobId: 'job_123456',
...
testSchema('CompleteMissionParamsSchema', schemas.CompleteMissionParamsSchema, {
  offerId: 'offer_789012'
}, [
  { offerId: '' }, // ID vuoto
]);
...
testSchema('CertifiedQuotesRequestSchema', schemas.CertifiedQuotesRequestSchema, {
  service_type: 'SPRAY',
  area_ha: 25.5,
  location_lat: 45.5,
  location_lng: 9.1
...
testSchema('DirectionsRequestSchema', schemas.DirectionsRequestSchema, {
  waypoints: [
    { lat: 45.5, lng: 9.1 },
    { lat: 45.6, lng: 9.2 }
  ],
