...
app.get('/', authMiddleware, async (c) => {
  try {
    ...
    // Ensure table exists
    const createTableQuery = process.env.DATABASE_URL?.startsWith('postgresql://') || process.env.DATABASE_URL?.startsWith('postgres://')
      ? `
...
app.post('/', authMiddleware, async (c) => {
  try {
    ...
    const nameValid = name && typeof name === 'string' && name.trim().length > 0;
    const polygonValid = polygon && Array.isArray(polygon) && polygon.length >= 3;
    const areaValid = area_ha !== undefined && area_ha !== null && !isNaN(Number(area_ha)) && Number(area_ha) > 0;

    if (!nameValid || !polygonValid || !areaValid) {
      console.log('❌ Validation failed:', { nameValid, polygonValid, areaValid });
      return c.json({ 
        error: 'Missing or invalid required fields',
        details: {
          name: nameValid ? 'valid' : 'invalid or empty',
          polygon: polygonValid ? 'valid' : 'must be array with at least 3 points',
    ...
    if (isPostgreSQL) {
      ...
      const fieldResponse = {
        ...
        location_json: newField.location_json ? JSON.parse(newField.location_json) : null
      };
