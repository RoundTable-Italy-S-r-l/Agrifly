...
app.post('/directions', async (c) => {
  try {
    ...
    try {
      ...
      return c.json({ error: error.message || 'Invalid coordinate format' }, 400);
    }
    ...
    console.log('🔑 GraphHopper API Key:', GRAPHHOPPER_API_KEY ? 'Present' : 'Missing');

    if (!GRAPHHOPPER_API_KEY) {
      console.warn('⚠️ No GraphHopper API key found, falling back to distance calculation');
    ...
    if (!response.ok || !data.paths || data.paths.length === 0) {
      console.warn('⚠️ GraphHopper API failed, falling back to distance calculation');
      const distance = calculateDistance(origin, destination);
      return c.json({
        distance: {
          text: `${distance.toFixed(1)} km`,
    ...
    return c.json({ error: 'Internal server error during routing calculation' }, 500);
  }
});
