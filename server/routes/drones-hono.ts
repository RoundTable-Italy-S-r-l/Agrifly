...
app.get("/:id", async (c) => {
  try {
        ...
        if (Array.isArray(glbFiles) && glbFiles.length > 0) {
          ...
          if (
            ...
            try {
              ...
              console.warn(
                "⚠️ Supabase Storage unavailable, trying local file:",
                e,
              );
    ...
    if (row.specs_core_json) {
      try {
        specs_core_json =
          typeof row.specs_core_json === "string"
    ...
    if (row.specs_extra_json) {
      try {
        specs_extra_json =
          typeof row.specs_extra_json === "string"
    ...
    if (
      (!images_json ||
        (Array.isArray(images_json) && images_json.length === 0)) &&
      row.id
    ) {
      try {
        ...
        if (productId.includes("t25p")) possiblePaths.push("images/t25p");
          ...
          productId.includes("mavic-3-m") ||
          productId.includes("mavic3m")
        )
          possiblePaths.push("images/mavic-3-m");

        if (modelName.includes("t25p")) possiblePaths.push("images/t25p");
        else if (modelName.includes("t25") && !modelName.includes("t25p"))
          possiblePaths.push("images/t25");
          ...
          modelName.includes("mavic-3-m") ||
          modelName.includes("mavic3m")
        )
          possiblePaths.push("images/mavic-3-m");
        ...
        if (!supabaseKey) {
          console.log(
            "⚠️ SUPABASE_SERVICE_ROLE_KEY non configurata - caricamento dinamico immagini disabilitato per",
            row.model,
          );
        }
        ...
        if (supabaseUrl && supabaseKey && possiblePaths.length > 0) {
          console.log(
            "🔄 Tentativo caricamento dinamico immagini per",
            row.model,
          );
          try {
            ...
            for (const imgPath of possiblePaths) {
              ...
              if (!error && files && files.length > 0) {
                ...
                if (imageFiles.length > 0) {
                  ...
                  console.log(
                    `✅ Caricate ${dynamicImages.length} immagini dinamicamente per ${row.model} da bucket Supabase (${imgPath})`,
                  );
            ...
            console.warn(
              "Errore caricamento immagini dinamiche dal bucket:",
              e,
            );
          }
        }
          ...
          try {
            ...
            if (
              ...
              if (fs.existsSync(localPath)) {
                ...
                return typeof img === "string"
                  ? `/api/drones/images/${relativePath}`
                  : { ...img, url: `/api/drones/images/${relativePath}` };
              }
            }
          }
        }
    ...
    if (
      (!videos_json ||
        (Array.isArray(videos_json) && videos_json.length === 0)) &&
      row.id
    ) {
      try {
        ...
        if (productId.includes("t25p")) possiblePaths.push("videos/t25p");
          ...
          possiblePaths.push("videos/mavic-3-m");

        if (modelName.includes("t25p")) possiblePaths.push("videos/t25p");
        else if (modelName.includes("t25") && !modelName.includes("t25p"))
          possiblePaths.push("videos/t25");
        ...
        if (!supabaseKey) {
          console.log(
            "⚠️ SUPABASE_SERVICE_ROLE_KEY non configurata - caricamento dinamico video disabilitato per",
            row.model,
          );
        }
        ...
        if (supabaseUrl && supabaseKey && possiblePaths.length > 0) {
          console.log(
            "🔄 Tentativo caricamento dinamico video per",
            row.model,
            "- paths:",
            possiblePaths,
          );
              ...
              if (!error && files && files.length > 0) {
                ...
                if (videoFiles.length > 0) {
                  const dynamicVideos = videoFiles.map((file) => {
                    ...
                    const mimeType = file.name.toLowerCase().endsWith(".mp4")
                      ? "video/mp4"
                  ...
                  console.log(
                    `✅ Caricati ${dynamicVideos.length} video dinamicamente per ${row.model} da bucket Supabase (${videoPath})`,
                  );
    ...
    manuals_pdf_json = manuals_pdf_json.map((manual: any) => {
      const manualUrl =
        typeof manual === "string"
          ? manual
          : manual.url || manual.filename || manual;
      if (
        typeof manualUrl === "string" &&
        ...
        try {
          // PRIORITÀ: Usa Supabase Storage
          const storagePath =
            manualUrl.startsWith("/manuals/") || manualUrl.startsWith("/pdf/")
          ...
          if (
            ...
            if (fs.existsSync(localPath)) {
              ...
              return typeof manual === "string"
                ? `/api/drones/manuals/${relativePath}`
                : { ...manual, url: `/api/drones/manuals/${relativePath}` };
            }
          }
        }
      }
...
app.get("/:id/metrics", async (c) => {
  try {
    ...
    const purposes = (product.purposes || []).filter(
    ...
    try {
      specs_core_json =
        typeof product.specs_core_json === "string"
    ...
    if (!Array.isArray(specs_core_json) || specs_core_json.length === 0) {
      console.error(
        "❌ [METRICS] specs_core_json non disponibile o vuoto per:",
        product.id,
      );
      return c.json(
        { error: "Product specs_core_json not available or empty" },
        400,
      );
    }

    console.log(
      "✅ [METRICS] specs_core_json trovato,",
      specs_core_json.length,
      "specs",
    );
    ...
    const allMetricsByPurpose: Record<
      string,
      Array<{ productId: string; metrics: Record<string, number | null> }>
    > = {};
    ...
    for (const purpose of purposes) {
      ...
      console.log(
        `🔍 [METRICS] Trovati ${allProductsResult.rows.length} prodotti con purpose ${purpose}`,
      );
      ...
      allMetricsByPurpose[purpose] = allProductsResult.rows.map((row) => {
        ...
        try {
          ...
          console.warn(
            `❌ [METRICS] Errore parsing specs_core_json per ${row.id}:`,
            e,
          );
        }
    ...
    if (allProductsMetrics.length === 0) {
      return c.json(
        {
          error: `No products found with purpose ${primaryPurpose} for comparison`,
        },
        404,
      );
    }
    ...
    Object.entries(currentProductMetrics).forEach(([key, value]) => {
      if (value !== null && minMax[key]) {
        normalizedMetrics[key] = normalizeValue(
          value,
          minMax[key].min,
          minMax[key].max,
        );
    ...
    const relevantClusters = (
      CLUSTERS_BY_PURPOSE[primaryPurpose] || Object.keys(METRIC_CLUSTERS)
    ).slice(0, 6);
    const clusterMetrics: Record<
      string,
      Array<{
        key: string;
        label: string;
...
app.get("/manuals/*", async (c) => {
  try {
    ...
    console.log("✅ Serving manual PDF file:", {
      path: fullPath,
      size: fileContent.length,
    });
