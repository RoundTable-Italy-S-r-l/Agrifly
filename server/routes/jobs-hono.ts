  ...
  try {
    ...
    const jobs = result.rows.map((job) => {
      ...
      try {
        ...
        console.warn(
          `⚠️ [GET JOBS] Error parsing location_json for job ${job.id}:`,
          e,
        );
        locJson = null;
      }
...
app.get("/operator/jobs", authMiddleware, async (c) => {
  try {
    ...
    const jobsWithOfferStatus = await Promise.all(
      result.rows.map(async (job) => {
        try {
          ...
          const existingOfferStatus = hasAnyOffer
            ? anyOfferResult.rows[0].status
            : null;
          ...
          try {
            ...
            console.warn(
              `⚠️ [OPERATOR JOBS] Error parsing location_json for job ${job.id}:`,
              parseError.message,
            );
            locJson = null;
          }
          ...
          return {
            ...
            buyer_org: {
              id: job.buyer_org_id,
              legal_name:
                job.buyer_org_legal_name || "Organizzazione sconosciuta",
            },
            can_offer: canOffer,
          ...
          console.error(
            `❌ [OPERATOR JOBS] Error processing job ${job.id}:`,
            jobError,
          );
    ...
    const validJobs = jobsWithOfferStatus.filter(
...
app.get("/:jobId", authMiddleware, async (c) => {
  try {
    ...
    if (!isBuyer && !isOpen) {
      return c.json(
        { error: "Non hai i permessi per visualizzare questo job" },
        403,
      );
    }
    ...
    try {
      ...
      console.warn(
        `⚠️ [GET JOB] Error parsing location_json for job ${jobId}:`,
        e,
      );
      locJson = null;
    }
    ...
    const offers = offersResult.rows.map((offer: any) => {
      ...
      try {
        if (offer.pricing_snapshot_json) {
          pricingSnapshot =
            typeof offer.pricing_snapshot_json === "string"
              ? JSON.parse(offer.pricing_snapshot_json)
              : offer.pricing_snapshot_json;
        }
      } catch (e) {
        console.warn(
          `⚠️ [GET JOB] Error parsing pricing_snapshot_json for offer ${offer.id}:`,
          e,
        );
...
app.post(
  "/",
  authMiddleware,
  validateBody(CreateJobSchema, { transform: true }),
  async (c) => {
    try {
      // @ts-ignore - Hono context typing issue
      const user = c.get("user") as any;
    ...
    `,
        ...
        try {
          if (newJob.location_json) {
            locJson =
              typeof newJob.location_json === "string"
                ? JSON.parse(newJob.location_json)
                : newJob.location_json;
          }
        } catch (e) {
          console.warn(
            `⚠️ [CREATE JOB] Error parsing location_json for job ${newJob.id}:`,
            e,
          );
          locJson = null;
        }
...
app.post(
  "/:jobId/offers",
  authMiddleware,
  validateBody(CreateJobOfferSchema, { transform: true }),
  async (c) => {
    try {
      ...
      console.log("📝 [CREATE OFFER] Creazione job offer:", {
        jobId,
      ...
      let jobResult;
      try {
        jobResult = await query(
          "SELECT id, status, buyer_org_id FROM jobs WHERE id = $1",
          [jobId],
        );
        console.log("✅ [CREATE OFFER] Job query result:", {
          rowsCount: jobResult?.rows?.length || 0,
        });
      } catch (jobQueryError: any) {
        console.error(
          "❌ [CREATE OFFER] Errore query job:",
      ...
      const job = jobResult.rows[0];
      console.log("📋 [CREATE OFFER] Job trovato:", {
        id: job.id,
        status: job.status,
      });
      ...
      try {
        ...
        console.log("✅ [CREATE OFFER] Existing offers query result:", {
          count: existingOfferResult?.rows?.length || 0,
        });
      } catch (existingOfferError: any) {
        console.error(
          "❌ [CREATE OFFER] Errore query offerte esistenti:",
      ...
      if (
        existingOfferResult &&
        existingOfferResult.rows &&
        existingOfferResult.rows.length > 0
      ) {
        console.log(
          "❌ [CREATE OFFER] Offerta già esistente:",
          existingOfferResult.rows[0],
        );
        return c.json(
          { error: "Operator already submitted an active offer for this job" },
          400,
        );
      }
      ...
      if (pricing_snapshot_json) {
        pricingSnapshotStr =
          typeof pricing_snapshot_json === "string"
      ...
      console.log("📦 [CREATE OFFER] Valori per INSERT:", {
        ...
        pricingSnapshotStr: pricingSnapshotStr
          ? pricingSnapshotStr.substring(0, 100)
          : null,
      ...
      const proposedStart =
        proposed_start && proposed_start.trim() !== "" ? proposed_start : null;
      const proposedEnd =
        proposed_end && proposed_end.trim() !== "" ? proposed_end : null;
      ...
      console.log(
        "📤 [CREATE OFFER] Values types:",
        insertValues.map((v) => typeof v),
      );
      ...
      console.log(
        "📥 [CREATE OFFER] Recupero offerta creata con ID:",
        actualOfferId,
      );
      let offerResult;
      try {
        offerResult = await query("SELECT * FROM job_offers WHERE id = $1", [
          actualOfferId,
        ]);
      ...
      if (!offerResult || !offerResult.rows || offerResult.rows.length === 0) {
        ...
        return c.json(
          { error: "Failed to create offer - offer not found after insert" },
          500,
        );
      }
      ...
      if (error.code)
        console.error("❌ [CREATE OFFER] Error code:", error.code);
      if (error.errno)
        console.error("❌ [CREATE OFFER] Error errno:", error.errno);
      if (error.detail)
        console.error("❌ [CREATE OFFER] Error detail:", error.detail);
      if (error.hint)
        console.error("❌ [CREATE OFFER] Error hint:", error.hint);
      return c.json(
        { error: "Internal server error", message: error.message },
        500,
      );
...
app.get("/:jobId/offers", authMiddleware, async (c) => {
  try {
    ...
    console.log("📋 [GET JOB OFFERS] Richiesta offerte per job:", {
      jobId,
    ...
    const jobResult = await query(
      "SELECT id, buyer_org_id FROM jobs WHERE id = $1",
      [jobId],
    );
    if (jobResult.rows.length === 0) {
      return c.json({ error: "Job not found" }, 404);
    ...
    const offers = offersResult.rows.map((row: any) => ({
      ...
      pricing_snapshot_json: (() => {
        try {
          ...
          console.warn(
            `⚠️ [GET OFFERS] Error parsing pricing_snapshot_json for offer ${row.id}:`,
            e,
          );
          return null;
        }
...
app.post(
  "/:jobId/accept-offer/:offerId",
  authMiddleware,
  validateParams(AcceptOfferParamsSchema),
  async (c) => {
    try {
      // @ts-ignore - Hono context typing issue
      const user = c.get("user") as any;
      ...
      console.log("✅ [ACCEPT OFFER] Accettazione offerta:", {
        jobId,
        offerId,
      ...
      // Fetch the job
      const jobResult = await query(
        "SELECT id, buyer_org_id, status FROM jobs WHERE id = $1",
        [jobId],
      );
      if (jobResult.rows.length === 0) {
        return c.json({ error: "Job not found" }, 404);
      ...
      if (job.buyer_org_id !== user.organizationId) {
        console.log("❌ [ACCEPT OFFER] Unauthorized: buyer org mismatch", {
          jobBuyerOrgId: job.buyer_org_id,
          userOrgId: user.organizationId,
        });
    ...
    `,
      ...
      console.log("✅ [ACCEPT OFFER] Offerta accettata e booking creata:", {
        offerId,
        bookingId,
      });
...
app.put(
  ...
  async (c) => {
    try {
      ...
      console.log("🔄 [UPDATE OFFER] Aggiornamento offerta:", {
        jobId,
        offerId,
        operatorOrgId: user.organizationId,
      });
      ...
      if (offer.status !== "OFFERED") {
        return c.json(
          {
            error:
              "L'offerta non può essere modificata (stato: " +
              offer.status +
              ")",
          },
          400,
      ...
      console.log(
        "✅ [UPDATE OFFER] Offerta aggiornata con successo:",
        offerId,
      );
...
// IMPORTANTE: questa route deve essere PRIMA di /:orgId per evitare conflitti di routing
app.post(
  "/:jobId/withdraw-offer/:offerId",
  authMiddleware,
  validateParams(WithdrawOfferParamsSchema),
  async (c) => {
    try {
      // @ts-ignore - Hono context typing issue
      const user = c.get("user") as any;
      ...
      if (offer.operator_org_id !== user.organizationId) {
        return c.json(
          { error: "Non autorizzato a ritirare questa offerta" },
          403,
        );
      }
      ...
      if (offer.status !== "OFFERED") {
        return c.json(
          {
            error:
              "L'offerta non può essere ritirata (stato: " + offer.status + ")",
          },
          400,
        );
      }
...
app.get("/offers/:offerId/messages", authMiddleware, async (c) => {
  try {
    ...
    console.log("💬 [GET MESSAGES] User authenticated:", {
      userId: user.userId,
      orgId: user.organizationId,
    });
    ...
    if (!isBuyer && !isOperator) {
      console.log(
        "💬 ❌ Authorization failed: user is neither buyer nor operator",
      );
      return c.json(
        {
          error: "Unauthorized: You can only view messages for your own offers",
        },
        403,
      );
    }
    ...
    if (!isAccepted && !isAwarded) {
      ...
      return c.json(
        {
          error:
            "Chat is available only after the offer is accepted or awarded",
        },
        403,
      );
    }
    ...
    const messages = result.rows.map((msg) => ({
      ...
      sender_name:
        `${msg.first_name || "Utente"} ${msg.last_name || ""}`.trim(),
    }));

    console.log(
      "✅ [GET MESSAGES] Recuperati",
      messages.length,
      "messaggi per offerta",
    );
...
app.post(
  ...
  async (c) => {
    ...
    `,
      ...
      if (
        offer.buyer_org_id !== sender_org_id &&
        offer.operator_org_id !== sender_org_id
      ) {
        return c.json(
          {
            error:
              "Unauthorized: You can only send messages for your own offers",
          },
          403,
        );
      }
      ...
      if (offer.status !== "ACCEPTED" && offer.status !== "AWARDED") {
        return c.json(
          {
            error:
              "Messages can only be sent after the offer is accepted or awarded",
          },
          403,
        );
      }
...
app.put(
  ...
  async (c) => {
    ...
    `,
      ...
      if (
        offer.buyer_org_id !== reader_org_id &&
        offer.operator_org_id !== reader_org_id
      ) {
        return c.json({ error: "Unauthorized" }, 403);
      }
...
app.post(
  ...
  authMiddleware,
  validateParams(CompleteMissionParamsSchema),
  ...
  async (c) => {
    try {
      ...
      console.log("✅ [COMPLETE MISSION] Completamento missione per offerta:", {
        offerId,
        operatorOrgId: user.organizationId,
      });
    ...
    `,
      ...
      if (offer.status !== "AWARDED" && offer.status !== "ACCEPTED") {
        return c.json(
          {
            error:
              "L'offerta deve essere assegnata o accettata per completare la missione",
          },
          400,
        );
      }
      ...
      if (offer.operator_org_id !== user.organizationId) {
        return c.json(
          { error: "Non autorizzato a completare questa missione" },
          403,
        );
      }
      ...
      if (
        ...
        console.log(
          "⚠️ [COMPLETE MISSION] No booking found for offer, trying to find any booking for job:",
          jobId,
        );
        ...
        if (existingBookingResult.rows.length > 0) {
          console.log(
            "⚠️ [COMPLETE MISSION] Found existing booking for job, will update it",
          );
        }
      }
      ...
      if (existingBookingResult.rows.length > 0) {
        ...
        console.log(
          "📋 [COMPLETE MISSION] Booking dopo aggiornamento:",
          verifyResult.rows[0],
        );
      } else {
        // Create new booking
      ...
      `,
        ...
        console.log(
          "📋 [COMPLETE MISSION] Booking dopo creazione:",
          verifyResult.rows[0],
        );
      }
  ...
  try {
    ...
    const received = receivedOffersResult.rows.map((row: any) => {
      ...
      try {
        ...
        console.warn(
          `⚠️ [GET OFFERS] Error parsing location_json for offer ${row.id}:`,
          e,
        );
        locJson = null;
      }
    ...
    console.log(
      `✅ Job offers trovate: received=${received.length}, made=${made.length}`,
    );
