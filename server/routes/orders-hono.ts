  ...
  try {
    ...
    if (role === "seller" || role === "vendor") {
      ...
      const orgCheck = await query(
        "SELECT type, org_type FROM organizations WHERE id = $1",
        [orgId],
      );
      if (orgCheck.rows.length > 0) {
        ...
        isSeller =
          ...
          orgType === "vendor" ||
          orgType === "operator";
        console.log("🔍 Organization type check:", {
          orgId,
          orgType,
          isSeller,
        });
      ...
      result.rows.map(async (order) => {
        ...
        `;
          ...
          try {
            ...
            console.warn(
              `⚠️ [GET ORDERS] Error parsing shipping_address for order ${order.id}:`,
              e,
            );
          }
          ...
          try {
            ...
            console.warn(
              `⚠️ [GET ORDERS] Error parsing billing_address for order ${order.id}:`,
              e,
            );
          }
          ...
          console.error(
            `❌ [GET ORDERS] Error processing order ${order.id}:`,
            orderError,
          );
...
app.post(
  ...
  async (c) => {
    try {
      ...
      const { cartId, shippingAddress, billingAddress, customerNotes } =
        validatedBody;
    ...
    `;
      ...
      if (vendorOrgIds.length === 0) {
        return c.json(
          { error: "Nessun vendor trovato per i prodotti nel carrello" },
          400,
        );
      }
      ...
      const quantity = Math.max(1, parseInt(item.qty) || 1);
    ...
    `;
      ...
      const orderValues = [
        ...
        sellerOrgId,
        null, // quote_id
        "PAID", // order_status - default PAID
        "PAID", // status (duplicato per sicurezza)
      ...
      for (const line of orderLines) {
        ...
        `,
          ...
          `,
            ...
            console.log(
              `✅ Inventory aggiornato per SKU ${line.sku_id}: riservati +${line.quantity} presso vendor ${vendorId}`,
            );
          } else {
          ...
          console.error(
            `❌ Errore aggiornamento inventory per SKU ${line.sku_id}:`,
  ...
  async (c) => {
    try {
      ...
      const orderCheck = await query(
        "SELECT id, seller_org_id FROM orders WHERE id = $1",
        [orderId],
      );
    ...
    `;
      ...
      const affectedRows =
        updateResult.rowCount !== undefined
          ? updateResult.rowCount
          : updateResult.changes || 0;
      if (affectedRows === 0) {
        return c.json(
          { error: "Order not found or already in this status" },
          404,
        );
      }
      ...
      if (
        ...
        console.log(
          "📦 Aggiornando inventory per spedizione/consegna ordine:",
          orderId,
        );
      ...
      `,
        ...
        for (const line of orderLinesResult.rows) {
          ...
          `,
            ...
            console.log(
              `✅ Inventory aggiornato per SKU ${line.sku_id}: spediti -${line.quantity} dal vendor ${line.seller_org_id}`,
            );
          } catch (inventoryError: any) {
            console.error(
              `❌ Errore aggiornamento inventory per SKU ${line.sku_id}:`,
    ...
    try {
      ...
      if (
        ...
        return c.json(
          {
            error:
              "Unauthorized: You can only send messages for your own orders",
          },
          403,
        );
      }
