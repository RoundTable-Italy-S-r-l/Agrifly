...
app.use("*", async (c, next) => {
  console.log(
    "🔵 [SERVICES SUB-APP] Request received:",
    c.req.method,
    c.req.path,
  );
...
app.get("/:orgId", authMiddleware, async (c: any) => {
  try {
    ...
    console.log(
      "💰 [GET RATE CARDS] User object completo:",
      JSON.stringify(user, null, 2),
    );
    console.log(
      "💰 [GET RATE CARDS] User organizationId:",
      user?.organizationId,
    );
    ...
    if (!user || user.organizationId !== orgId) {
      console.log(
        "❌❌❌ [GET RATE CARDS] UNAUTHORIZED - RESTITUISCO 403 ❌❌❌",
      );
    ...
    const rateCards = result.rows.map((row: any) => ({
      ...
      travel_fixed_cents: row.travel_fixed_cents
        ? parseInt(row.travel_fixed_cents)
        : 0,
      travel_rate_per_km_cents: row.travel_rate_per_km_cents
        ? parseInt(row.travel_rate_per_km_cents)
        : null,
      hilly_terrain_multiplier: row.hilly_terrain_multiplier
        ? parseFloat(row.hilly_terrain_multiplier)
        : null,
      hilly_terrain_surcharge_cents: row.hilly_terrain_surcharge_cents
        ? parseInt(row.hilly_terrain_surcharge_cents)
        : 0,
      ...
      hourly_operator_rate_cents: row.hourly_operator_rate_cents
        ? parseInt(row.hourly_operator_rate_cents)
        : null,
      seasonal_multipliers_json: row.seasonal_multipliers_json || null,
      risk_multipliers_json: row.risk_multipliers_json || null,
      show_company_only:
        row.show_company_only !== undefined
          ? typeof row.show_company_only === "boolean"
            ? row.show_company_only
            : row.show_company_only === 1 || row.show_company_only === "true"
          : false,
    ...
    console.log(
      "✅ [GET RATE CARDS] Recuperate",
      rateCards.length,
      "rate cards",
    );
    console.log(
      "✅ [GET RATE CARDS] Rate cards:",
      JSON.stringify(rateCards, null, 2),
    );
    console.log(
      "💰 [GET RATE CARDS] ===========================================",
    );
...
app.get("/:orgId/:serviceType", async (c) => {
  try {
    ...
    if (!orgId || !serviceType) {
      return c.json(
        { error: "Organization ID and Service Type required" },
        400,
      );
    }
    ...
    try {
      ...
      if (existing.rows.length > 0) {
        ...
        if (custom_multipliers_json !== undefined) {
          ...
          updateValues.push(
            custom_multipliers_json === null
              ? null
              : typeof custom_multipliers_json === "string"
                ? custom_multipliers_json
                : JSON.stringify(custom_multipliers_json),
          );
        }
        if (custom_surcharges_json !== undefined) {
          updateFields.push(`custom_surcharges_json = $${paramIndex++}`);
          updateValues.push(
            custom_surcharges_json === null
              ? null
              : typeof custom_surcharges_json === "string"
                ? custom_surcharges_json
                : JSON.stringify(custom_surcharges_json),
          );
        }
        ...
        if (seasonal_multipliers_json !== undefined) {
          ...
          updateValues.push(
            seasonal_multipliers_json === null
              ? null
              : typeof seasonal_multipliers_json === "string"
                ? seasonal_multipliers_json
        ...
        if (risk_multipliers_json !== undefined) {
          ...
          updateValues.push(
            risk_multipliers_json === null
              ? null
              : typeof risk_multipliers_json === "string"
                ? risk_multipliers_json
        ...
        if (assigned_operator_ids !== undefined) {
          ...
          updateValues.push(
            assigned_operator_ids === null
              ? null
              : typeof assigned_operator_ids === "string"
                ? assigned_operator_ids
                : JSON.stringify(assigned_operator_ids),
          );
        }
        ...
        if (supported_model_codes !== undefined) {
          ...
          updateValues.push(
            supported_model_codes === null
              ? null
              : typeof supported_model_codes === "string"
                ? supported_model_codes
                : JSON.stringify(supported_model_codes),
          );
        }
        ...
        if (updateFields.length === 0) {
          ...
          return c.json(
            { error: "No fields to update", bodyKeys: Object.keys(body) },
            400,
          );
        }
      ...
      `;
        ...
        const rateCard = {
          ...
          show_company_only:
            row.show_company_only !== undefined
              ? typeof row.show_company_only === "boolean"
                ? row.show_company_only
                : row.show_company_only === 1 ||
                  row.show_company_only === "true"
              : false,
          assigned_operator_ids: row.assigned_operator_ids || null,
          operator_assignment_mode:
            row.operator_assignment_mode || "ASSIGNED_ONLY",
          service_area_set_id: row.service_area_set_id || null,
          crop_types: row.crop_types || null,
        };
        ...
        const validServiceTypes = ['MAPPING', 'SPRAY', 'SPREAD', 'SEEDING', 'SCOUTING'];
        if (!validServiceTypes.includes(service_type)) {
          console.log('❌ Invalid service_type:', service_type, 'valid types:', validServiceTypes);
...
// Temporarily disabled - has service_type enum issue
// app.put(
//   "/:orgId/:rateCardId",
//   authMiddleware,
//   validateBody(UpdateServiceSchema, { stripUnknown: true }),
//   async (c) => {
    try {
      const orgId = c.req.param("orgId");
    ...
    console.log(
      ...
      const checkQuery =
        "SELECT service_type FROM rate_cards WHERE id = $1 AND seller_org_id = $2";
    ...
    console.log(
      "✅ PUT Rate card exists with service_type:",
      existingServiceType,
    );

    // Validate existing service_type
    const validServiceTypes = ['MAPPING', 'SPRAY', 'SPREAD', 'SEEDING', 'SCOUTING'];
    if (!validServiceTypes.includes(existingServiceType)) {
      console.log("❌ Existing rate card has invalid service_type:", existingServiceType);
      return c.json({ error: "Rate card has invalid service type" }, 400);
    }

    // Validate that service_type is valid for update
    const validServiceTypes = ['MAPPING', 'SPRAY', 'SPREAD'];
    if (!validServiceTypes.includes(existingServiceType)) {
      console.log("❌ Invalid service_type found:", existingServiceType);
      return c.json({ error: 'Invalid service type in existing record' }, 400);
    }
    ...
    // Extract all possible fields - NOTE: service_type should NOT be in body for PUT /:orgId/:rateCardId
    let {
      service_type, // Extract and discard - not allowed in updates
      base_rate_per_ha_cents,
      min_charge_cents,
      travel_fixed_cents,
      travel_rate_per_km_cents,
    ...
    // Log if service_type was present (should not be)
    if (service_type !== undefined) {
      console.log("⚠️ service_type found in body but ignored:", service_type);
    }
    ...
    console.log("💰 PUT Body validated:", body);
...
app.put(
  ...
  async (c) => {
    try {
      ...
      console.log(
        "💰 PUT Update rate card by serviceType:",
        serviceType,
        "per org:",
        orgId,
        "data:",
