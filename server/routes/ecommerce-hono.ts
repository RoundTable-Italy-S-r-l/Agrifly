...
import {
  ...
  CreateAddressLegacySchema,
  UpdateAddressLegacySchema,
...
app.post("/create-tables", async (c) => {
  try {
    ...
    return c.json(
      { error: "Errore creazione tabelle", message: error.message },
      500,
    );
  }
});
  ...
  try {
    ...
    if (cartResult.rows.length === 0) {
      ...
      if (userId && orgId) {
        ...
        return c.json(
          { error: "Cannot create cart without userId, sessionId, or orgId" },
          400,
        );
      }
  ...
  try {
    ...
    if (existingItem.rows.length > 0) {
      ...
      const priceToUse =
        unitPriceCents !== null
          ? unitPriceCents
          : existingItem.rows[0].unit_price_cents;
    ...
    await query("UPDATE shopping_carts SET updated_at = NOW() WHERE id = $1", [
      cartId,
    ]);
  ...
  async (c) => {
    try {
      ...
      console.log("🔍 Check item result:", {
        itemId,
        found: checkItem.rows.length,
        currentQuantity: checkItem.rows[0]?.quantity,
      });
...
app.delete(
  "/cart/items/:itemId",
  validateParams(DeleteItemParamsSchema),
  async (c) => {
    try {
...
app.post("/cart/migrate", validateBody(MigrateCartSchema), async (c) => {
  try {
    ...
    if (guestCartResult.rows.length === 0) {
      ...
      return c.json({
        message: "Nessun carrello guest da migrare",
        migrated: false,
      });
    }
...
async function ensureWishlistProductIdColumn() {
  ...
  try {
    ...
    if (
      ...
      !err.message?.includes('relation "wishlist_items" does not exist')
    ) {
...
app.post(
  "/addresses",
  authMiddleware,
  validateBody(CreateAddressLegacySchema),
  async (c) => {
    try {
      ...
      const generateId = () => {
        ...
        return id.length >= 25
          ? id.substring(0, 30)
          : id.padEnd(25, random).substring(0, 30);
      };
...
app.put(
  "/addresses/:addressId",
  authMiddleware,
  validateBody(UpdateAddressLegacySchema),
  async (c) => {
    try {
      ...
      if (updates.is_default) {
        const addressResult = await query(
          "SELECT org_id, type FROM addresses WHERE id = $1",
          [addressId],
        );
        if (addressResult.rows.length > 0) {
          await query(
            ...
            [
              addressResult.rows[0].org_id,
              addressResult.rows[0].type,
              addressId,
            ],
          );
        }
...
app.delete(
  "/addresses/:addressId",
  authMiddleware,
  validateParams(DeleteAddressParamsSchema),
  async (c) => {
    try {
