...
export const isUsingPostgreSQL =
  hasPostgresConfig ||
  DATABASE_URL.startsWith("postgresql://") ||
  DATABASE_URL.startsWith("postgres://");
export const isUsingSQLite =
  !isUsingPostgreSQL && (DATABASE_URL.startsWith("file:") || isDevelopment);

// Resend API Key (per email)
export const RESEND_API_KEY = process.env.RESEND_API_KEY || null;
export const RESEND_FROM_EMAIL =
  process.env.RESEND_FROM_EMAIL ||
  "DJI Agriculture <noreply@dji-agriculture.com>";
...
if (isDevelopment) {
  ...
  console.log(
    "  Database:",
    isUsingSQLite ? "SQLite" : isUsingPostgreSQL ? "PostgreSQL" : "Unknown",
  );
