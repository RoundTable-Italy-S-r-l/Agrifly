...
if (!supabaseUrl || !supabaseKey) {
  console.error('❌ Errore: SUPABASE_URL e SUPABASE_ANON_KEY devono essere configurati nelle variabili d\'ambiente');
  process.exit(1);
}
...
async function findT70PPdfs() {
  try {
    ...
    console.log(`📄 Trovati ${t70pPdfs.length} PDF potenzialmente del T70P nella root:`);
    t70pPdfs.forEach(pdf => {
      const url = `${supabaseUrl}/storage/v1/object/public/Media FIle/${pdf.name}`;
      console.log(`  - ${pdf.name} (${(pdf.metadata?.size / 1024 / 1024)?.toFixed(2) || 'N/A'} MB)`);
      console.log(`    URL: ${url}`);
    });

    // Cerca sottocartelle
    const folders = [...new Set(rootFiles.filter(f => f.name.includes('/')).map(f => f.name.split('/')[0]))];
    console.log(`\n📂 Sottocartelle trovate: ${folders.join(', ')}`);

    // Esplora cartelle che potrebbero contenere PDF
    const foldersToCheck = ['manuals', 'pdfs', 'docs', 'documents', 't70p', 'T70P'];
    for (const folder of foldersToCheck.filter(f => folders.includes(f))) {
      ...
      try {
        ...
        const folderT70p = folderFiles.filter(file =>
    ...
    console.log(`Trovati ${allT70pFiles.length} file che contengono "t70p" o "t70":`);
