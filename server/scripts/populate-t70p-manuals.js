...
async function populateT70PManuals() {
  try {
    ...
    if (!supabaseUrl) {
      console.error(
        "❌ Errore: SUPABASE_URL deve essere configurato nelle variabili d'ambiente",
      );
      return;
    }
    ...
    for (const manual of possibleManuals) {
      try {
        ...
        if (response.ok) {
          ...
          console.log(
            `❌ ${manual.filename} - NON ESISTE (${response.status})`,
          );
        }
      } catch (error) {
    ...
    console.log(
      `✅ Database aggiornato. Righe modificate: ${updateResult.rowCount}`,
    );
