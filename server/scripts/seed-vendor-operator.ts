...
async function seedVendorOperator() {
  try {
    ...
    if (!organization) {
      ...
      console.log(
        "✅ Created vendor-operator organization:",
        organization.legal_name,
      );
    } else {
      console.log(
        "✅ Using existing vendor-operator organization:",
        organization.legal_name,
      );
    }
    ...
    if (existingMembership) {
      console.log(
        "✅ Membership already exists with role:",
        ...
        operatorUser = await prisma.user.create({
          data: {
            ...
            password_hash: await hashPassword(
              process.env.SEED_OPERATOR_PASSWORD || "operator123",
            ),
            email_verified: true,
            status: "ACTIVE",
          },
    ...
    console.log(
      `   Password: ${process.env.SEED_VENDOR_PASSWORD || "vendor123"} (configura SEED_VENDOR_PASSWORD per cambiarla)`,
    );
    console.log("");
