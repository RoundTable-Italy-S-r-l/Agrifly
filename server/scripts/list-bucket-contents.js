...
const POSSIBLE_BUCKET_NAMES = ['Media File', 'Media FIle', 'media-file', 'mediafile', 'MediaFile'];
...
async function listBucketFiles(prefix = '', bucketName = BUCKET_NAME) {
  ...
  throw new Error(`Impossibile accedere al bucket "${bucketName}" con nessun metodo di encoding`);
}
...
async function findBucket() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(`   ⚠️  Impossibile listare bucket (HTTP ${response.status})`);
    }
  } catch (e) {
...
async function main() {
  try {
    ...
    if (!foundBucket) {
      console.error('❌ Bucket non trovato. Verifica il nome del bucket nelle variabili d\'ambiente.');
      process.exit(1);
    }
