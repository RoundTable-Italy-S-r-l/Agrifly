...
const DRONES = [
  {
    ...
    targetUse:
      "Grandi estensioni, Cerealicoltura intensiva. Efficienza massima.",
    imageUrl:
      "https://cdn.builder.io/api/v1/image/assets%2F2465e073f7c94097be8616ce134014fe%2F58d74d38d1fb4075a2b3a226cc229907?format=webp&width=800",
  ...
  },
  {
    ...
    imageUrl:
      "https://cdn.builder.io/api/v1/image/assets%2F2465e073f7c94097be8616ce134014fe%2Fd8d2c5c643ae4c44b415ab8d453b6b93?format=webp&width=800",
    glbFile: null,
    specs: {
      tank: "30L",
      battery: "N/A",
  ...
  },
  {
    ...
    imageUrl:
      "https://cdn.builder.io/api/v1/image/assets%2F2465e073f7c94097be8616ce134014fe%2Fbc07e146eb984c448fb3fe46a05699c8?format=webp&width=800",
  ...
  },
  {
    ...
    imageUrl:
      "https://cdn.builder.io/api/v1/image/assets%2F2465e073f7c94097be8616ce134014fe%2Fd5ca49f288ac47d8932ade07a9b066ae?format=webp&width=800",
...
async function main() {
  ...
  for (const drone of DRONES) {
    ...
    const coreSpecsPath = path.join(
      DJI_KB_PATH,
      "products_specs_core",
      `${drone.id}.json`,
    );
    const extraSpecsPath = path.join(
      DJI_KB_PATH,
      "products_specs_extra",
      `${drone.id}.json`,
    );
    ...
    const imagesJson = drone.imageUrl
      ? JSON.stringify([
          { url: drone.imageUrl, alt: drone.model, is_primary: true },
        ])
      : null;
    const glbFilesJson = drone.glbFile
      ? JSON.stringify([
          {
            url: drone.glbFile,
            filename: drone.glbFile.split("/").pop(),
            size: null,
          },
        ])
      : null;
    ...
    db.prepare(
      ...
      ON CONFLICT(id) DO UPDATE SET
        ...
        status = excluded.status
    `,
    ).run(skuId, productId, `DJI_${drone.id.toUpperCase()}`, "unit", "ACTIVE");

    console.log(
      `✅ Seeded ${drone.model} (${coreSpecs.length} core specs, ${extraSpecs.length} extra specs, ${manuals.length} manuals)`,
    );
  }
