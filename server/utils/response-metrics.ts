 ...
 */
export async function calculateResponseEventsForConversation(
  ...
  if (convCheck.rows[0].status !== "OPEN") {
    console.log(
      `⚠️  Conversation ${conversationId} non è OPEN (status: ${convCheck.rows[0].status})`,
    );
    return 0;
  }
  ...
  for (const [blockId, blockLast] of blockLastMessages.entries()) {
    ...
    if (nextBlock) {
      const responseSeconds = Math.floor(
        (nextBlock.created_at.getTime() - blockLast.created_at.getTime()) /
          1000,
      );
 ...
 */
export async function aggregateResponseMetrics(
 ...
 */
export async function processAllOpenConversations(
  daysBack: number = 7,
): Promise<{
  processed: number;
