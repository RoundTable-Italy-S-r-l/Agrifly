...
export default function Carrello() {
  ...
  const { data: cartData, isLoading: cartLoading } = useQuery({
    ...
    queryFn: () => {
      ...
      if (!currentOrgId && !sessionId) throw new Error('Session ID o Organizzazione richiesta');
  ...
  const { data: wishlistData, isLoading: wishlistLoading } = useQuery({
    ...
    queryFn: () => {
      if (!currentOrgId || currentOrgId === 'guest_org') throw new Error('Organizzazione non trovata');
  ...
  const cartTotal = cartItems.reduce((total, item) => {
    const itemPrice = item.unit_price_cents && item.unit_price_cents > 0 ? item.unit_price_cents : 0;
      ...
      <BuyerLayout>
        <div className="max-w-4xl mx-auto">
          <div className="text-center py-12">
            ...
            <h2 className="text-xl font-semibold text-slate-900 mb-2">Seleziona un'organizzazione</h2>
            <p className="text-slate-600 mb-6">Devi selezionare un'organizzazione per visualizzare il carrello</p>
            <Link to="/login">
  ...
  return (
    <BuyerLayout>
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Carrello e Wishlist</h1>
          <p className="text-slate-600">Gestisci i tuoi prodotti preferiti e procedi all'acquisto</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="p-6 border-b border-slate-200">
                <div className="flex items-center gap-3">
                  ...
                  <span className="bg-slate-100 text-slate-600 px-2 py-1 rounded-full text-sm">
                    {cartItems.length} prodott{cartItems.length !== 1 ? 'i' : 'o'}
                  </span>
                </div>
              </div>
              ...
              <div className="p-6">
                {cartLoading ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-medium text-slate-900 mb-2">Il carrello è vuoto</h3>
                      ...
                      <div key={item.id} className="flex items-center gap-4 p-4 bg-slate-50 rounded-lg">
                        ...
                        <div className="flex-1">
                          <h3 className="font-medium text-slate-900">
                            {item.product_name || item.product_model || `Prodotto #${item.sku_id}`}
                          </h3>
                ...
                <div className="p-6 border-b border-slate-200">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="bg-red-100 text-red-600 px-2 py-1 rounded-full text-sm">
                      {wishlistItems.length} prodott{wishlistItems.length !== 1 ? 'i' : 'o'}
                    </span>
                  </div>
                </div>
                    ...
                    <div className="space-y-4">
                      {wishlistItems.map((item: WishlistItem) => (
                        <div key={item.id} className="flex items-center gap-4 p-4 bg-red-50 rounded-lg">
                          ...
                          <div className="flex-1">
                            <h3 className="font-medium text-slate-900">Prodotto #{item.sku_id}</h3>
                          </div>
            ...
            {currentUser && currentOrgId && currentOrgId !== 'guest_org' && !sessionId && (
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="p-6 border-b border-slate-200">
                  <div className="flex items-center gap-3">
                    <CreditCard className="w-6 h-6 text-emerald-600" />
                    <h2 className="text-xl font-semibold text-slate-900">I tuoi ordini</h2>
                      ...
                      {orders.map((order: Order) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 text-sm text-slate-600">
                            ...
                            <span>{order.order_lines?.length || 0} prodotto{(order.order_lines?.length || 0) !== 1 ? 'i' : ''}</span>
                            {order.order_lines && order.order_lines.length > 0 && (
                              <>
                                <span>•</span>
                                <span>{order.order_lines[0].product_name || order.order_lines[0].product_model || 'Prodotto'}</span>
                                {order.order_lines.length > 1 && (
                                  <span>e altri {order.order_lines.length - 1}</span>
                                )}
                              </>
                            )}
                          </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
              <h3 className="text-lg font-semibold text-slate-900 mb-4">Riepilogo ordine</h3>

              <div className="space-y-3 mb-6">
                <div className="flex justify-between">
                  <span className="text-slate-600">Prodotti ({cartItems.length})</span>
              ...
              {!currentUser && (
                <p className="text-xs text-slate-500 text-center mt-2">
                  Accedi per completare l'acquisto e salvare la cronologia ordini
                </p>
              )}
            </div>
