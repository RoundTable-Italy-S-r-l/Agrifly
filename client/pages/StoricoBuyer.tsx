...
import { getMyJobs, acceptJobOffer, getBookings, payBooking, Job } from '@/lib/api';
import { toast } from 'sonner';

export default function StoricoBuyer() {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState<string>('all');
  const [sortBy, setSortBy] = useState<'date' | 'offers' | 'area' | 'name'>('date');
  ...
  const getServiceLabel = (serviceType: string) => {
    switch (serviceType) {
      case 'SPRAY': return 'Trattamento fitosanitario';
      case 'SPREAD': return 'Spandimento fertilizzanti';
      case 'MAPPING': return 'Mappatura territoriale';
  ...
  const SortIcon = ({ field }: { field: 'date' | 'offers' | 'area' | 'name' }) => {
    if (sortBy !== field) {
  ...
  const filteredJobs = jobs
    .filter((job: Job) => {
      const matchesSearch = job.field_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                          parseLocation(job.location_json).toLowerCase().includes(searchTerm.toLowerCase()) ||
                          getServiceLabel(job.service_type).toLowerCase().includes(searchTerm.toLowerCase());
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-slate-600 mt-1">Tutti i servizi richiesti e il loro stato</p>
          </div>
          <Link
              ...
              <tbody className="bg-white divide-y divide-slate-200">
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={7} className="px-6 py-12 text-center">
                      ...
                      <h3 className="text-lg font-medium text-slate-900 mb-2">
                        {jobs.length === 0 ? 'Nessun servizio pubblicato' : 'Nessun servizio trovato'}
                      </h3>
                  ...
                  filteredJobs.map((job: Job) => {
                    const booking = bookingsData?.bookings?.find((b: any) => b.job_id === job.id);
                    ...
                    return (
                      <tr key={job.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-sm text-slate-900">{getServiceLabel(job.service_type)}</span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-sm text-slate-900">{parseFloat(job.area_ha)?.toFixed(1) || 'N/A'} ha</span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
