...
export default function Missioni() {
  ...
  const [offerToWithdraw, setOfferToWithdraw] = useState<{
    jobId: string;
    offerId: string;
  } | null>(null);
  ...
  const { data: availableJobs = { jobs: [] }, isLoading: loadingJobs } =
    useQuery({
      ...
      enabled: !!(
        userPermissions?.can_access_bookings ||
        userPermissions?.can_complete_missions
      ), // Operator/dispatcher possono vedere job disponibili
    });
  ...
  const statusFilteredOffers =
    offersToShow?.filter((offer) => {
      // Escludi sempre le offerte accettate dalla tab "Offerte Inviate"
      if (offer.status === "AWARDED") {
        console.log(
          '🚫 Esclusa offerta AWARDED dalla tab "Offerte Inviate":',
          offer.id,
        );
        return false;
      }
  ...
  const excludedAwarded =
    offersToShow?.filter((o) => o.status === "AWARDED").length || 0;
  if (excludedAwarded > 0) {
    console.log(
      `🔍 Filtro applicato: ${excludedAwarded} offerta/e AWARDED escluse dalla tab "Offerte Inviate"`,
    );
  }

  const filteredOffers =
    statusFilteredOffers?.filter(
      (offer) =>
        offer &&
        offer.job &&
        offer.job.buyer_org &&
  ...
  const handleRejectOffer = async (jobId: string, offerId: string) => {
    try {
      ...
      const response = await fetch(
        `/api/jobs/${jobId}/reject-offer/${offerId}`,
        {
          method: "POST",
          headers: {
  ...
  const pendingOffers =
    offersToShow?.filter((o) => o.status === "OFFERED").length || 0;
  ...
  const acceptedOffersList =
    offersToShow?.filter((offer) => {
      ...
      const hasCompletedBooking = (bookingsData.bookings || []).some(
        (booking: any) =>
  ...
  const rejectedOffers =
    offersToShow?.filter(
      (o) => o.status === "DECLINED" || o.status === "WITHDRAWN",
    ).length || 0;
        ...
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="text-xs text-slate-500 mt-2">
              Org ID: {currentOrgId || "null"}, Permissions:{" "}
              {userPermissions ? "calculated" : "null"}
            </p>
          </div>
        </div>
      </AdminLayout>
  ...
  const handleConfirmWithdraw = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          error.message || error.error || "Errore nel ritiro dell'offerta",
        );
      }
          ...
          >
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Sei sicuro di voler ritirare questa offerta? Il job tornerà ad
                  essere disponibile per nuove offerte.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel
          ...
          <TabsList className="grid w-full grid-cols-4">
            ...
            <TabsTrigger value="offerte" className="flex items-center gap-2">
              ...
              {(() => {
                ...
                const nonAwardedCount =
                  offersToShow?.filter((o) => o.status !== "AWARDED").length ||
                  0;
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <Badge variant="outline">
                                {serviceTypeConfig[job.service_type]?.label ||
                                  job.service_type}
                              </Badge>
                            </div>
                          ...
                          <div className="flex flex-col gap-2">
                            {job.can_offer ? (
                              ...
                              <Badge variant="secondary">
                                Offerta{" "}
                                {job.existing_offer_status?.toLowerCase()}
                              </Badge>
                            ) : (
                  ...
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-right w-[120px]">
                        {isOperatorView ? "Preventivo" : "Offerta"}
                      </TableHead>
