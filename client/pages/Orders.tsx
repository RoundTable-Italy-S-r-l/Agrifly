...
type KanbanStatus = 'PAID' | 'SHIPPED' | 'FULFILLED' | 'CANCELLED' | 'PROBLEMATIC';
...
const kanbanColumns: Array<{
  ...
  { status: 'PAID', label: 'Pagato', color: 'bg-yellow-50 border-yellow-200', icon: DollarSign },
  { status: 'SHIPPED', label: 'Spedito', color: 'bg-blue-50 border-blue-200', icon: Truck },
  { status: 'FULFILLED', label: 'Completato', color: 'bg-green-50 border-green-200', icon: CheckCircle },
  { status: 'CANCELLED', label: 'Annullato', color: 'bg-red-50 border-red-200', icon: XCircle },
  { status: 'PROBLEMATIC', label: 'Problematico', color: 'bg-orange-50 border-orange-200', icon: AlertTriangle },
];

// Componente per card ordine draggable
function OrderCard({ order, onOpenDetail }: { order: Order; onOpenDetail: (order: Order) => void }) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  const style = {
    ...
    transition: transition || 'transform 200ms cubic-bezier(0.2, 0, 0.2, 1), opacity 150ms ease-out',
...
function KanbanColumn({
  ...
  return (
    <div className="space-y-3 w-full" ref={setNodeRef}>
      {/* Header colonna */}
      <div className={`flex items-center justify-between p-3 rounded-lg border-2 ${column.color} ${
        isOver 
        ...
        <div className="space-y-3 min-h-[200px]">
          {orders.map((order, index) => (
            <div
              ...
              style={{
                animation: landingOrderId === order.id ? undefined : `fadeInSlide 0.3s ease-out ${index * 30}ms both`,
              }}
            >
...
export default function Orders() {
  ...
  const updateOrderStatusMutation = useMutation({
    mutationFn: async ({ orderId, status }: { orderId: string; status: KanbanStatus }) => {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorData = { error: errorText || `Errore ${response.status}: ${response.statusText}` };
        }
    ...
    onMutate: async ({ orderId, status }) => {
      ...
      const previousOrders = queryClient.getQueryData<Order[]>(['orders', currentOrgId]);
    ...
    onError: (err, variables, context) => {
      ...
      if (context?.previousOrders) {
        queryClient.setQueryData(['orders', currentOrgId], context.previousOrders);
      }
      ...
      <AdminLayout>
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="text-sm text-slate-500 mt-2">Seleziona un'organizzazione per visualizzare gli ordini</p>
          </div>
        </div>
      </AdminLayout>
    );
    ...
    <AdminLayout>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium text-slate-600">Ordini da evadere</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-yellow-600">{kpis.toFulfill}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium text-slate-600">In spedizione</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-blue-600">{kpis.shipped}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-3">
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium text-slate-600">Valore periodo</CardTitle>
            </CardHeader>
            <CardContent>
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-full sm:max-w-2xl overflow-y-auto">
        <SheetHeader>
          ...
          <SheetDescription>
            {statusLabels[order.order_status] || order.order_status} · {formatDate(order.created_at)} · {formatPrice(order.total_cents, order.currency)}
          </SheetDescription>
        </SheetHeader>
