    ...
    if (job) {
      ...
      if (awardedOffer && currentOrgId) {
        console.log('💬 Chat should be visible in DettagliJobBuyer:', {
          ...
          operatorOrgId: awardedOffer.operator_org?.id || awardedOffer.operator_org_id
        });
      }
    }
  }, [job, currentOrgId]);
      ...
      <div className="max-w-7xl mx-auto px-8 space-y-4">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8">
                ...
                <Badge className={`${getStatusColor(job.status)} text-sm px-3 py-1`}>
                  ...
                  <span className="ml-1 font-medium">
                    {job.status === 'OPEN' ? 'Aperto' :
                     job.status === 'AWARDED' ? 'Aggiudicato' :
                     job.status === 'DONE' || job.status === 'COMPLETED' ? 'Completato' :
                     job.status === 'EXPIRED' ? 'Scaduto' :
                     job.status === 'CANCELLED' ? 'Annullato' : 'Sconosciuto'}
                  </span>
                </Badge>
                {job.offers && job.offers.length > 0 && (
                  <Badge variant="secondary" className="bg-slate-100 text-slate-700 text-sm px-3 py-1">
                    {job.offers.length} offerta{job.offers.length !== 1 ? 'e' : ''}
                  </Badge>
                )}
              </div>
            </div>
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between items-center py-2 border-b border-slate-100">
                ...
                <span className="font-mono text-xs bg-slate-100 px-2 py-1 rounded">{job?.id?.slice(-8) || 'N/A'}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-100">
              ...
              <div className="flex justify-between items-center py-2 border-b border-slate-100">
                ...
                <span className="text-slate-900 font-semibold">{job.area_ha.toFixed(1)} ha</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-100">
            ...
            {job.offers && job.offers.length > 0 ? (
              <div className="space-y-3">
                {job.offers.map((offer) => (
                  <div key={offer.id} className="bg-white border border-slate-200 rounded-lg p-4">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4 text-xs text-slate-600 mb-2">
                          ...
                          {offer.proposed_start && offer.proposed_end && (
                            <span>
                              {new Date(offer.proposed_start).toLocaleDateString('it-IT', { day: 'numeric', month: 'short' })} -
                              {new Date(offer.proposed_end).toLocaleDateString('it-IT', { day: 'numeric', month: 'short' })}
                            </span>
                          )}
                        </div>

                        {offer.provider_note && (
                          <p className="text-slate-700 text-sm mb-2">"{offer.provider_note}"</p>
                        )}
                      </div>
                      ...
                      <div className="text-right">
                        ...
                        <div className="flex flex-col gap-3 items-end">
                          <Badge className={`${getOfferStatusColor(offer.status)} text-xs px-3 py-1`}>
                            {offer.status === 'OFFERED' ? 'In Attesa' :
                             offer.status === 'AWARDED' ? 'Accettata' :
                             offer.status === 'DECLINED' ? 'Rifiutata' : offer.status}
                          </Badge>

                          {job.status === 'OPEN' && offer.status === 'OFFERED' && (
                            <Button
                              size="sm"
                            ...
                            >
                              {acceptOfferMutation.isPending ? '...' : 'Accetta'}
                            </Button>
                          )}
                        </div>
                      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            {(() => {
              try {
                const constraints = job.constraints_json ? JSON.parse(job.constraints_json) : null;
                const hasReadableNotes = constraints && constraints.notes && constraints.notes.trim();
                  ...
                  return (
                    <div className="bg-slate-50 rounded-xl shadow-sm border border-slate-200 p-8">
                      ...
                      <div className="bg-white rounded-lg p-4 border-l-4 border-slate-300">
                        <p className="text-slate-800 leading-relaxed">{constraints.notes}</p>
                      </div>
                    </div>
                  );
                }
        ...
        <AlertDialog open={!!offerToAccept} onOpenChange={(open) => !open && setOfferToAccept(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Conferma accettazione offerta</AlertDialogTitle>
              <AlertDialogDescription>
                Sei sicuro di voler accettare questa offerta? Non potrai più accettare altre offerte per questo job.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              ...
              >
                {acceptOfferMutation.isPending ? 'Accettazione...' : 'Conferma accettazione'}
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
