  ...
  return (
    <div>
      <div className={`flex items-center gap-2 ${compact ? "mb-2" : "mb-3"}`}>
        <Navigation
          className={`${compact ? "w-4 h-4" : "w-5 h-5"} text-slate-600`}
        />
        <span
          className={`font-medium ${compact ? "text-sm text-slate-600" : "text-slate-700"}`}
        >
      ...
      <div className="flex items-center justify-between">
        <div className={`flex items-center gap-4 ${compact ? "text-sm" : ""}`}>
          <div className="flex items-center gap-1">
            <MapPin
              className={`${compact ? "w-3 h-3" : "w-4 h-4"} text-slate-500`}
            />
        ...
        <div className={`flex items-center ${compact ? "gap-1" : "gap-2"}`}>
          <Button
            ...
            size="sm"
            onClick={() =>
              window.open(routingData.navigation_links.google_maps, "_blank")
            }
          ...
          <Button
            ...
            size="sm"
            onClick={() =>
              window.open(routingData.navigation_links.waze, "_blank")
            }
            className="text-xs h-7"
          >
            <ExternalLink className="w-3 h-3 mr-1" />
      ...
      {routingData.fallback && (
        <p className="text-xs text-amber-600 mt-1">
          *Distanza in linea d'aria.{" "}
          {compact ? "GraphHopper" : "Installa GraphHopper"} per percorsi
          stradali precisi.
        </p>
      )}
    </div>
...
export default function OfferDetail() {
  ...
  const [quoteEstimate, setQuoteEstimate] =
    useState<QuoteEstimateResponse | null>(null);
  ...
  const {
    ...
    queryFn: () =>
      currentOrgId
        ? fetchJobOffers(currentOrgId).catch((err) => {
  ...
  const {
    data: operatorJobsData = { jobs: [] },
    isLoading: loadingOperatorJobs,
  } = useQuery({
  ...
  const { data: bookingsData = { bookings: [] }, isLoading: loadingBookings } =
    useQuery({
  ...
  // Prima cerca se offerId corrisponde a un'offerta esistente
  const offer = allOffers.find((o) => o.id === offerId);

  // Se non trova offerta, potrebbe essere un ID di job - cerca il job corrispondente
  ...
  // Logica per determinare il job da mostrare
  let job = null;
  let effectiveOffer = offer;

  if (offer) {
    // Se abbiamo un'offerta esistente, trova il job associato
  ...
  // Debug logging
  console.log("🔍 [OFFER DEBUG] Resolution result:", {
    offerId,
    foundOffer: !!offer,
    ...
    jobId: job?.id,
    offerJobId: effectiveOffer?.job_id,
  });
  ...
  const booking =
    effectiveOffer && effectiveOffer.status === "AWARDED" && bookingsData?.bookings
      ? bookingsData.bookings.find((b: any) => b.accepted_offer_id === effectiveOffer.id)
      : null;
  ...
  useEffect(() => {
    if (offer && offer.status === "AWARDED") {
      ...
      console.log(
        "📋 All Accepted Offer IDs:",
        bookingsData?.bookings?.map((b: any) => b.accepted_offer_id) || [],
      );
  ...
  const { data: messagesData = { messages: [] }, isLoading: loadingMessages } =
    useQuery({
      ...
      enabled:
        ...
        booking?.job?.conversation?.status === "OPEN",
  ...
  const fieldCoordinates = useMemo(() => {
    ...
    const polygon =
      ...
      job.polygon ||
      (job.location_json &&
      typeof job.location_json === "object" &&
      job.location_json.polygon
        ? job.location_json.polygon
        : null);
  ...
  const { data: routingData, isLoading: loadingRouting } = useQuery({
    queryKey: [
      "directions",
      operatorLocation,
      jobWithCenter?.field_center_lng,
      jobWithCenter?.field_center_lat,
    ],
    queryFn: () => {
      if (
        !operatorLocation ||
      ...
      return getDirections(operatorLocation, {
        lng: jobWithCenter.field_center_lng,
        lat: jobWithCenter.field_center_lat,
      });
    },
    enabled:
      !!operatorLocation &&
  ...
  useEffect(() => {
    if (existingOffer && !manualPriceOverride) {
      setOfferFormData({
        ...
        proposed_start: existingOffer.proposed_start
          ? existingOffer.proposed_start.split("T")[0]
          : "",
        proposed_end: existingOffer.proposed_end
          ? existingOffer.proposed_end.split("T")[0]
          : "",
    ...
    const calculateQuote = async () => {
      ...
      try {
        ...
        const rateCard = rateCards.find(
          (rc: any) => rc.service_type === jobWithCenter.service_type,
        );

        if (!rateCard) {
          console.warn(
            "No rate card found for service type:",
            jobWithCenter.service_type,
          );
        ...
        navigator.geolocation.getCurrentPosition(
          ...
          async (error) => {
            ...
            try {
              ...
              if (
                serviceConfig.base_location_lat &&
                serviceConfig.base_location_lng
              ) {
                setOperatorLocation({
                ...
                console.log(
                  "✅ Usando indirizzo base configurato:",
              ...
              console.warn(
                "❌ Errore caricamento service config:",
                configError,
              );
            }
  ...
  */
  ...
  const confirmCompleteMission = async () => {
    ...
    try {
      console.log("🚀 [COMPLETE MISSION] Starting mission completion:", {
        offerId: offer.id,
      });
      ...
      console.log(
        "✅ [COMPLETE MISSION] Mission completed successfully:",
        result,
      );
      ...
      console.error(
        "❌ [COMPLETE MISSION] Errore nel completamento della missione:",
        error,
      );
  ...
  useEffect(() => {
    const loadLeaflet = async () => {
      try {
        ...
        const { MapContainer, TileLayer, Polygon, Popup } =
  ...
  useEffect(() => {
    if (offer && LeafletComponents && mapRef.current) {
      ...
      const map = L.map(mapRef.current).setView([45.4642, 9.19], 10); // Centro Italia
      ...
      if (offer.job.field_polygon) {
        try {
          ...
          const polygonCoords =
            typeof offer.job.field_polygon === "string"
          ...
          if (Array.isArray(polygonCoords) && polygonCoords.length > 0) {
            ...
            // Aggiungi popup
            polygon.bindPopup(
              `<b>${offer.job.field_name}</b><br/>Area: ${offer.job.area_ha} ha`,
            );
          }
        } catch (error) {
        ...
        try {
          ...
          if (Array.isArray(polygon) && polygon.length > 0) {
            ...
            polygon.forEach((point: any) => {
              ...
              if (Array.isArray(point) && point.length >= 2) {
                ...
                if (!isNaN(lng) && !isNaN(lat) && lng >= -180 && lng <= 180 && lat >= -90 && lat <= 90) {
                  // Determina se è [lng, lat] o [lat, lng] basandosi sui valori
                  if (Math.abs(lng) > 90) {
                    // Probabilmente [lng, lat] - lng fuori dal range normale lat
                    totalLng += lng;
                    totalLat += lat;
                  } else {
                    // Probabilmente [lat, lng]
                    totalLat += lng;
                    totalLng += lat;
                  }
                  validPoints++;
            ...
            if (validPoints > 0) {
              ...
              // Verifica che i valori siano numeri validi prima di chiamare toFixed
    ...
    const handleCreateOffer = async () => {
      ...
      try {
        ...
        const totalCents = Math.round(
          parseFloat(offerFormData.total_cents) * 100,
        );
        if (totalCents <= 0) {
        ...
        <div className="max-w-4xl mx-auto p-6">
          <div className="text-center py-12">
            <div className="text-slate-500 mb-4">
              Offerta o Job non trovato
            </div>
            <div className="text-sm text-slate-400 mb-6 max-w-md mx-auto">
              L'ID "{offerId}" potrebbe riferirsi a un job di test o a un'offerta che non esiste più.
              Verifica che l'URL sia corretta o torna alla lista delle missioni.
            </div>
  ...
  const serviceConfig = serviceTypeConfig[
    offer.job.service_type as keyof typeof serviceTypeConfig
  ] || { label: offer.job.service_type, icon: "🔧" };
  ...
  return (
    <AdminLayout>
      <div className="max-w-4xl mx-auto p-6 space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-xl font-bold text-slate-900 mb-2">
                {offer.job.field_name}
              </div>
              <div className="text-sm text-slate-600">
                <span className="font-semibold text-slate-900">
                  {Number(offer.job.area_ha).toFixed(1)}
                </span>{" "}
                ettari
              </div>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              ...
              {offer.job.treatment_type && (
                <div className="text-sm text-slate-600">
                  <span className="font-medium">Trattamento:</span>{" "}
                  {offer.job.treatment_type}
                </div>
              )}
            </CardContent>
              ...
              {routingData ? (
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-2">
                    * Distanze calcolate con percorso stradale ottimale
                  </p>
                </div>
              ) : (
                <div className="text-center py-4">
                  ...
                  <p className="text-xs text-slate-400">
                    * Senza GraphHopper API: distanza in linea d'aria
                  </p>
                </div>
              )}
            </CardContent>
              ...
              {offer.proposed_start && offer.proposed_end ? (
                <>
                  ...
                  <div className="text-xs text-slate-500 mb-2">
                    {new Date(offer.proposed_start).toLocaleTimeString(
                      "it-IT",
                  ...
                  <div className="text-xs text-slate-500">
                    {new Date(offer.proposed_end).toLocaleTimeString("it-IT", {
                      hour: "2-digit",
                      minute: "2-digit",
                    })}
                  </div>
        ...
        {/* Riquadro Buyer Info Flash - Sempre visibile quando abbiamo buyer info */}
        {job?.buyer_org && (
            ...
            <CardContent>
              <div className="space-y-3">
                ...
                <Button
                  variant="outline"
                  onClick={() =>
                    navigate(`/operators/${job.buyer_org.id}`)
                  }
                  className="w-full"
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card>
            ...
            <CardContent>
              <div className="font-semibold text-lg text-slate-900 mb-1">
                {job?.buyer_org?.legal_name || "Non disponibile"}
              </div>
              <div className="text-sm text-slate-600">
                Organizzazione richiedente
              </div>
          ...
          {effectiveOffer && (
            <Card>
              ...
              <CardContent>
                <div className="font-semibold text-lg text-slate-900 mb-1">
                  {effectiveOffer.operator_org?.legal_name || "Non disponibile"}
                </div>
                <div className="text-sm text-slate-600">
            ...
            <CardContent className="space-y-4">
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                ...
                <p className="text-sm text-green-700">
                  La tua offerta è stata accettata dal cliente. Completa la
                  missione quando il lavoro è stato eseguito.
                </p>
              </div>
              ...
              {booking && (
                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-200">
                  <div>
                    ...
                    <Badge
                      variant={
                        booking.status === "DONE" ? "default" : "secondary"
                      }
                    >
          ...
          return (
            shouldShowChat && (
              <JobOfferChat
                ...
                buyerOrgId={
                  offer.job?.buyer_org?.id ||
                  (offer.job as any)?.buyer_org_id ||
                  ""
                }
                operatorOrgId={
                  offer.operator_org_id ||
                  (offer.operator_org as any)?.id ||
                  currentOrgId
                }
              />
            )
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Sei sicuro di voler completare questa missione? Il lavoro sarà
