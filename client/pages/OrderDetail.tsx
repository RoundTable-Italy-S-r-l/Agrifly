      ...
      <BuyerLayout>
        <div className="max-w-6xl mx-auto py-12">
          <div className="text-center">
            ...
            <h2 className="text-xl font-semibold text-slate-900 mb-2">Ordine non trovato</h2>
            <p className="text-slate-600 mb-6">L'ordine richiesto non esiste o non è accessibile</p>
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="text-right">
              ...
              <p className="text-xs text-slate-500 mt-1">
                Pagamento: {order.payment_status === 'PAID' ? 'Pagato' : order.payment_status}
              </p>
            </div>
          </div>
          ...
          {order.tracking_number && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="font-semibold text-blue-900">Numero di tracking</p>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
              ...
              <div className="space-y-4">
                {order.order_lines.map((line) => (
                  <div key={line.id} className="flex items-center gap-4 p-4 bg-slate-50 rounded-lg">
                    <div className="w-16 h-16 bg-slate-200 rounded-lg flex items-center justify-center">
                      <Package className="w-8 h-8 text-slate-400" />
                    </div>
                    <div className="flex-1">
                      <h3 className="font-medium text-slate-900">
                        {line.product_name || line.product_model || `Prodotto #${line.sku_id}`}
                      </h3>
                      ...
                      {line.sku_code && (
                        <p className="text-xs text-slate-500">SKU: {line.sku_code}</p>
                      )}
                    </div>
                    <div className="text-right">
                      <p className="text-sm text-slate-600">Quantità: {line.quantity}</p>
                      <p className="font-semibold text-slate-900">
                        €{((line.line_total_cents || 0) / 100).toFixed(2)}
                      </p>
                      <p className="text-xs text-slate-500">
                        €{((line.unit_price_cents || 0) / 100).toFixed(2)} cadauno
                      </p>
                    </div>
                  </div>
                ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {order.shipping_address && (
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                  ...
                  <div className="space-y-1 text-sm text-slate-600">
                    {order.shipping_address.name && <p className="font-medium text-slate-900">{order.shipping_address.name}</p>}
              ...
              {order.billing_address && (
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                  ...
                  <div className="space-y-1 text-sm text-slate-600">
                    {order.billing_address.name && <p className="font-medium text-slate-900">{order.billing_address.name}</p>}
                    ...
                    {order.billing_address.vat_number && <p className="mt-2">P.IVA: {order.billing_address.vat_number}</p>}
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 sticky top-4">
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{shipping === 0 ? 'Gratuita' : `€${shipping.toFixed(2)}`}</span>
                </div>
