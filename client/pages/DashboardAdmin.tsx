...
export default function DashboardAdmin() {
  ...
  const kpiData = orderStats
    ? {
        ...
        pendingBookings: activeMissions.filter((m) => m.status === "scheduled")
          .length,
      }
    : {
        hardwareRevenue: 0,
  ...
  const transformedRecentOrders = (orderStats?.recentOrders || [])
    .slice(0, 3)
    .map((order) => ({
      id: order.id,
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-slate-600 mt-1">
              Panoramica completa del business e gestione operativa
            </p>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900 mt-1">
                  €{" "}
                  {kpiData.hardwareRevenue.toLocaleString("it-IT", {
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900 mt-1">
                  €{" "}
                  {kpiData.serviceCommissions.toLocaleString("it-IT", {
                    minimumFractionDigits: 0,
                    maximumFractionDigits: 0,
                  })}
                </p>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900 mt-1">
                  {kpiData.completedOrdersThisMonth}
                </p>
                <span className="text-xs text-slate-500 mt-2">Questo mese</span>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">
                  Ultimi ordini di vendita hardware
                </p>
              </div>
            ...
            <div className="divide-y divide-slate-100">
              {transformedRecentOrders.length > 0 ? (
                transformedRecentOrders.map((order) => (
                  <div
                    key={order.id}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">
                  Servizi di trattamento in corso
                </p>
              </div>
              <button className="text-sm text-blue-600 hover:text-blue-700 font-medium">
                Gestisci →
                ...
                <div key={mission.id} className="p-4">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <div className="text-sm text-slate-600">
                        {mission.operator} • {mission.area} ha
                      </div>
                    </div>
                    <span
