...
const DroneDetail = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "specs" | "gallery" | "manuals" | "faq" | "reviews"
  >("overview");
  ...
  const productId = drone
    ? (drone as any).productId || (drone as any).id
    : null;
  ...
  const addToCartFromVendorMutation = useMutation({
    mutationFn: async (vendor: ProductVendor) => {
      if (!currentOrgId) {
        throw new Error(
          "Devi essere loggato per aggiungere prodotti al carrello. Effettua il login prima.",
        );
      }
  ...
  const addToWishlistMutation = useMutation({
    mutationFn: async () => {
      if (!currentOrgId) {
        throw new Error(
          "Devi essere loggato per aggiungere prodotti alla wishlist. Effettua il login prima.",
        );
      }
  ...
  const coreSpecs = Array.isArray(droneAny.specsCore)
    ? droneAny.specsCore
    : Array.isArray(droneAny.specs_core_json)
      ? droneAny.specs_core_json
      : [];
  const extraSpecs = Array.isArray(droneAny.specsExtra)
    ? droneAny.specsExtra
    : Array.isArray(droneAny.specs_extra_json)
      ? droneAny.specs_extra_json
      : [];
  const images = Array.isArray(droneAny.images) ? droneAny.images : [];
  const videos = Array.isArray((droneAny as any).videos)
    ? (droneAny as any).videos
    : [];
  const manuals = Array.isArray(droneAny.manuals)
    ? droneAny.manuals
    : Array.isArray(droneAny.manuals_pdf_json)
      ? droneAny.manuals_pdf_json
      : [];
          ...
          <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-xl p-6 border border-emerald-200">
            <div className="flex items-baseline gap-3 mb-2">
              <span className="text-4xl font-bold text-slate-900">
                €{" "}
                {typeof drone.price === "number"
                  ? drone.price.toLocaleString("it-IT")
                  : "N/D"}
              </span>
            </div>
                ...
                >
                        ...
                        <div className="text-right space-y-1">
                          ...
                          <div className="flex flex-col items-end gap-1">
                            ...
                            <span className="text-xs text-slate-500">
                              Da €
                              {vendor.offer.originalPrice.toLocaleString(
                                "it-IT",
                                {
                          ...
                          <div className="text-xl font-bold text-emerald-600">
                            €
                            {vendor.price.toLocaleString("it-IT", {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}
                          </div>
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    {currentOrgId ? (
                      <button
                        onClick={() =>
                          addToCartFromVendorMutation.mutate(vendor)
                        }
                        disabled={
                    ...
                    <div className="mt-3 pt-3 border-t border-slate-200 space-y-2">
                      <div className="text-sm">
                        ...
                        <p className="text-slate-600">
                          {vendor.leadTimeDays
                            ? `${vendor.leadTimeDays} giorni lavorativi`
                            : "Da confermare"}
                        </p>
                      </div>
                      {vendor.notes && (
                        ...
                        {videos.map((video: any, idx: number) => (
                          ...
                          >
                            <video
                              ...
                              poster={
                                typeof video === "object" && video.thumbnail
                                  ? video.thumbnail
                                  : undefined
                              }
                            >
          ...
          {activeTab === "reviews" && (
            <div className="space-y-6">
              <div className="text-center mb-8">
                ...
                <p className="text-slate-600">
                  Le recensioni verranno implementate a breve. Qui verranno
                  mostrati i punteggi per categoria e i commenti degli
                  acquirenti.
                </p>
              </div>
              <div className="bg-slate-50 rounded-lg p-8 border border-slate-200">
                <p className="text-slate-600 text-center">
                  Funzionalità in arrivo: sistema di recensioni con punteggi per
                  categoria e commenti.
                </p>
              </div>
            </div>
          )}
          ...
          {activeTab === "faq" && (
            <div className="space-y-6">
              <div className="text-center mb-8">
                ...
                <p className="text-slate-600">
                  Risposte alle domande più comuni sui nostri droni agricoli
                </p>
              </div>
              ...
              <div className="space-y-4">
                <div className="bg-slate-50 rounded-lg p-6 border border-slate-200">
                  ...
                  <p className="text-slate-700 leading-relaxed">
                    La durata della batteria dipende dal modello e dal carico di
                    lavoro. In condizioni ottimali, i nostri droni possono
                    volare da 10 a 25 minuti. La batteria agli ioni di litio ad
                    alta densità garantisce prestazioni costanti durante tutto
                    il volo.
                  </p>
                </div>
                ...
                <div className="bg-slate-50 rounded-lg p-6 border border-slate-200">
                  ...
                  <p className="text-slate-700 leading-relaxed">
                    L'area di copertura dipende dal modello e dalla
                    configurazione. I nostri droni possono coprire da 10 a 50
                    ettari per ora, con una larghezza di lavoro che va da 3 a 13
                    metri. L'efficienza può raggiungere fino a 40 ettari/ora con
                ...
                <div className="bg-slate-50 rounded-lg p-6 border border-slate-200">
                  <h3 className="font-semibold text-slate-900 mb-3 flex items-center gap-2">
                    <CheckCircle size={20} className="text-emerald-600" />È
                    difficile da pilotare?
                  </h3>
                  <p className="text-slate-700 leading-relaxed">
                    Assolutamente no! I nostri droni sono dotati di sistemi di
                    pilotaggio automatico avanzati. Con l'app dedicata, puoi
                    pianificare missioni precise, impostare percorsi automatici
                    e monitorare il volo in tempo reale. Anche gli operatori
                    alle prime armi possono utilizzarli dopo un breve training.
                  </p>
                </div>
                ...
                <div className="bg-slate-50 rounded-lg p-6 border border-slate-200">
                  ...
                  <p className="text-slate-700 leading-relaxed">
                    I nostri droni sono equipaggiati con sistemi di sicurezza
                    avanzati. In caso di perdita di segnale, il drone attiva
                    automaticamente la modalità RTH (Return to Home) e torna al
                    punto di partenza. Inoltre, i sistemi radar e di visione
                    artificiale garantiscono un volo sicuro anche in condizioni
                ...
                <div className="bg-slate-50 rounded-lg p-6 border border-slate-200">
                  ...
                  <p className="text-slate-700 leading-relaxed">
                    I nostri droni richiedono una manutenzione minima. I
                    componenti principali hanno una durata elevata e sono
                    progettati per resistere alle condizioni agricole più
                    difficili. Offriamo contratti di manutenzione preventiva e
                ...
                <div className="bg-slate-50 rounded-lg p-6 border border-slate-200">
                  ...
                  <p className="text-slate-700 leading-relaxed">
                    Sì! I nostri droni utilizzano tecnologie di precisione che
                    riducono significativamente l'uso di prodotti chimici. La
                    distribuzione mirata permette di applicare solo dove
                    necessario, proteggendo l'ambiente e riducendo i costi
                    operativi fino al 30%.
                  </p>
                </div>
                ...
                <div className="bg-slate-50 rounded-lg p-6 border border-slate-200">
                  ...
                  <p className="text-slate-700 leading-relaxed">
                    Certamente! Offriamo diverse configurazioni per adattarci
                    alle tue esigenze specifiche. Puoi scegliere tra serbatoi di
                    diverse capacità, sistemi di distribuzione multipli, e
                    accessori speciali per diversi tipi di colture e terreni.
                  </p>
                </div>
                ...
                <div className="bg-slate-50 rounded-lg p-6 border border-slate-200">
                  ...
                  <p className="text-slate-700 leading-relaxed">
                    Il ritorno sull'investimento varia dai 6 ai 24 mesi, a
                    seconda del modello e dell'utilizzo. I nostri clienti
                    tipicamente risparmiano sui costi di manodopera, riducono
                    l'uso di prodotti chimici e aumentano la produttività. Molti
                    clienti raggiungono il break-even entro il primo anno.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-emerald-50 rounded-lg p-6 border border-emerald-200 mt-8">
                ...
                <p className="text-emerald-800 mb-4">
                  Il nostro team di esperti è a tua disposizione per rispondere
                  a qualsiasi domanda sui nostri droni agricoli e aiutarti a
