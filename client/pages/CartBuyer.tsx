    ...
    <BuyerLayout>
      <div className="space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">Carrello & Wishlist</h1>
            <p className="text-slate-600 mt-1">Gestisci i tuoi acquisti e prodotti preferiti</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="bg-slate-50 px-6 py-4 border-b border-slate-200">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-lg font-semibold text-slate-900">Carrello ({cartItems.length} prodotti)</h2>
            </div>
          </div>
          ...
          <div className="p-6">
            {cartLoading ? (
              <div className="flex items-center justify-center py-12">
                ...
                <span className="ml-2 text-slate-600">Caricamento carrello...</span>
              </div>
            ) : cartItems.length === 0 ? (
              <div className="text-center py-12">
                <ShoppingCart className="w-16 h-16 text-slate-300 mx-auto mb-4" />
                <h3 className="text-lg font-medium text-slate-900 mb-2">Il carrello è vuoto</h3>
                <p className="text-slate-600 mb-6">Aggiungi prodotti dal catalogo per iniziare gli acquisti</p>
                <Link
                  to="/"
                  ...
                  {cartItems.map((item: CartItem) => (
                    <div key={item.id} className="flex items-center gap-4 p-4 bg-slate-50 rounded-lg">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-slate-600">{item.product_model} • {item.brand}</p>
                        <p className="text-sm text-slate-500">SKU: {item.sku_code}</p>
                      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="bg-slate-50 px-6 py-4 border-b border-slate-200">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-lg font-semibold text-slate-900">Wishlist ({wishlistItems.length} prodotti)</h2>
            </div>
          </div>
          ...
          <div className="p-6">
            {wishlistLoading ? (
              <div className="flex items-center justify-center py-8">
                ...
                <span className="ml-2 text-slate-600">Caricamento wishlist...</span>
              </div>
            ) : wishlistItems.length === 0 ? (
              <div className="text-center py-8">
                <Heart className="w-12 h-12 text-slate-300 mx-auto mb-4" />
                <h3 className="text-lg font-medium text-slate-900 mb-2">La wishlist è vuota</h3>
                <p className="text-slate-600">Aggiungi prodotti preferiti dal catalogo</p>
              </div>
            ) : (
                ...
                {wishlistItems.map((item: WishlistItem) => (
                  <div key={item.id} className="border border-slate-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        <h4 className="font-medium text-slate-900">{item.product_name}</h4>
                        <p className="text-sm text-slate-600">{item.product_model}</p>
                    ...
                    {item.note && (
                      <p className="text-sm text-slate-600 mb-3 italic">"{item.note}"</p>
                    )}
