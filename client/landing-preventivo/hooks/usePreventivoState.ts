...
export function usePreventivoState() {
  ...
  const updateServiceConfig = useCallback((updates: Partial<ServiceConfiguration>) => {
    setState(prev => ({
      ...prev,
  ...
  const calculatePricing = useCallback(async () => {
    if (!state.gisData || !state.serviceConfig.treatment) {
      setState(prev => ({ ...prev, error: 'Dati mancanti per il calcolo del preventivo' }));
      return;
    }
    ...
    try {
      ...
      const service_type = state.serviceConfig.treatment.type === 'solid' ? 'SPREAD' : 'SPRAY';

      // Determine risk multiplier
      let risk_key: string | undefined;
      if (state.serviceConfig.hasObstacles && state.serviceConfig.isHillyTerrain) {
        risk_key = 'high';
      } else if (state.serviceConfig.hasObstacles || state.serviceConfig.isHillyTerrain) {
        risk_key = 'medium';
      }
  ...
  // Operator selection
  const setOperatorSelection = useCallback((operator: AvailableOperator | null, autoSelect: boolean = false) => {
    setState(prev => ({
      ...prev,
  ...
  // Validation
  const canProceedToStep = useCallback((targetStep: PreventivoStep): boolean => {
    switch (targetStep) {
      case 2:
        return !!state.gisData;
      case 3:
        return !!state.gisData && !!state.serviceConfig.category && !!state.serviceConfig.treatment && !!state.serviceConfig.selectedDrone;
      case 4:
        return !!state.pricing && (!!state.selectedOperator || state.autoSelectOperator);
      default:
        return true;
    }
  }, [state]);
