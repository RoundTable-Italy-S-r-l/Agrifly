...
import { ChevronLeft, Users, MapPin, Clock, Star, CheckCircle, Sparkles } from 'lucide-react';
import { fetchOperators } from '@/lib/api';
import { AvailableOperator, GisData, PricingBreakdown, ServiceConfiguration } from '../types/preventivo.types';
...
interface OperatorMatcherProps {
  ...
  onSelectOperator: (operator: AvailableOperator | null, autoSelect: boolean) => void;
  onBack: () => void;
  onProceed: () => void;
  className?: string;
}
...
function calculateOperatorAvailability(
  ...
  return operators
    .filter(op => op.status === 'ACTIVE' && op.service_tags.includes(serviceConfig.treatment?.type === 'solid' ? 'SPREAD' : 'SPRAY'))
    ...
    .map((op, index) => {
      ...
      const daysNeeded = Math.ceil(parseFloat(gisData.area) / (op.max_ha_per_day || 20));
...
export function OperatorMatcher({
  ...
  useEffect(() => {
    if (operators.length > 0 && serviceConfig.treatment) {
      const available = calculateOperatorAvailability(operators, gisData, serviceConfig, pricing);
  ...
  }, [operators, gisData, serviceConfig, pricing, selectedOperator, autoSelectOperator, onSelectOperator]);
        ...
        <div className="bg-slate-100 px-6 py-4 border-b border-slate-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-lg text-slate-900">Scegli il tuo operatore</h3>
                <p className="text-sm text-slate-600">Operatori qualificati disponibili nella tua zona</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-6">
          ...
          <div className="mb-6">
            <label className="flex items-center gap-3 p-4 border-2 rounded-lg cursor-pointer transition-all hover:border-emerald-300">
              <input
                ...
                className="sr-only"
              />
              <div className={`p-2 rounded-lg ${autoSelectOperator ? 'bg-emerald-50' : 'bg-slate-50'}`}>
                <Sparkles className={`w-5 h-5 ${autoSelectOperator ? 'text-emerald-600' : 'text-slate-600'}`} />
              </div>
              <div className="flex-1">
                <div className="font-semibold text-slate-900">Troviamo noi il miglior operatore per te</div>
                <div className="text-sm text-slate-600 mt-1">
                  Selezioniamo automaticamente l'operatore più adatto basandoci su disponibilità, distanza e recensioni
                </div>
              </div>
              {autoSelectOperator && <CheckCircle className="w-5 h-5 text-emerald-600" />}
            </label>
          </div>
          ...
          <div className="mb-4">
            <label className="flex items-center gap-3 mb-4">
              <input
                ...
                className="sr-only"
              />
              <span className={`font-semibold ${!autoSelectOperator ? 'text-emerald-700' : 'text-slate-600'}`}>
          ...
          {isLoading ? (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-slate-600">Ricerca operatori disponibili...</p>
            </div>
                ...
                >
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-bold text-slate-900">
                          ...
                          {index === 0 && <span className="ml-2 text-xs bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full">MIGLIOR MATCH</span>}
                        </h4>
                        <div className="flex items-center gap-4 text-sm text-slate-600 mt-1">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            {operator.rating?.toFixed(1)} ({operator.review_count} recensioni)
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="text-right">
                      <div className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${getAvailabilityColor(operator.availability_score)}`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div className="bg-slate-50 rounded-lg p-3">
                      <div className="text-xs text-slate-500 uppercase">Tariffa Giornaliera</div>
                      <div className="font-bold text-slate-900">{formatPrice(operator.daily_rate_cents)}</div>
                    </div>
                    <div className="bg-slate-50 rounded-lg p-3">
                      <div className="text-xs text-slate-500 uppercase">Spese Trasferta</div>
                      <div className="font-bold text-slate-900">{formatPrice(operator.travel_cost_cents)}</div>
                    </div>
                    <div className="bg-slate-50 rounded-lg p-3">
                      <div className="text-xs text-slate-500 uppercase">Giorni Stimati</div>
                      <div className="font-bold text-slate-900">{operator.estimated_days} giorni</div>
                    </div>
                  </div>
                  ...
                  {selectedOperator?.id === operator.id && (
                    <div className="mt-3 flex items-center gap-2 text-emerald-700">
                      ...
                      <span className="text-sm font-medium">Operatore selezionato</span>
                    </div>
                  )}
                </div>
              ))}
          ...
          {(selectedOperator || autoSelectOperator) && (
            <div className="mt-6 bg-emerald-50 border border-emerald-200 rounded-lg p-4">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="font-semibold text-emerald-800">Pronto per procedere</h4>
