...
export const authAPI = {
  ...
  login: async (email: string, password: string): Promise<AuthResponse> => {
    console.log(
      "🔐 [FRONTEND LOGIN] Sending request to:",
      `${API_BASE}/auth/login`,
    );
    console.log("🔐 [FRONTEND LOGIN] Request body:", {
      email,
    ...
    if (!response.ok) {
      try {
        ...
        console.log(
          "🔐 [FRONTEND LOGIN] Failed to parse error response:",
          parseError,
        );
    ...
    try {
      ...
      console.log(
        "🔐 [FRONTEND LOGIN] Failed to parse success response:",
        parseError,
      );
  ...
  requestPasswordReset: async (
    email: string,
  ): Promise<{ message: string; resetUrl?: string; warning?: string }> => {
