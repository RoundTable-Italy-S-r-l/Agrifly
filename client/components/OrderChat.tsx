...
import { fetchOrderMessages, sendOrderMessage, markOrderMessagesAsRead, type OrderMessage } from '@/lib/api';
...
export function OrderChat({ orderId, currentOrgId, currentUserId, buyerOrgId, sellerOrgId }: OrderChatProps) {
  ...
  const sendMessageMutation = useMutation({
    mutationFn: (text: string) => sendOrderMessage(orderId, {
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col h-[500px]">
      ...
      <div className="p-4 border-b border-slate-200">
        ...
        <p className="text-sm text-slate-600 mt-1">
          Comunica con {isBuyer ? 'il venditore' : 'il cliente'} riguardo a questo ordine
        </p>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="flex items-center justify-center h-full text-slate-500">
            <div className="text-center">
              ...
              <p className="text-xs text-slate-400 mt-1">Inizia la conversazione</p>
            </div>
          </div>
        ) : (
