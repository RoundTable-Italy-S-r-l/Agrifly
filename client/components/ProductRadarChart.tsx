...
function formatMetricValue(value: number | null, key: string): string {
  ...
  if (key.includes("capacity") && key.includes("mah"))
    return `${formatted} mAh`;
  if (key.includes("capacity")) return `${formatted} L`;
  if (key.includes("weight") || key.includes("payload") || key.includes("mtow"))
    return `${formatted} kg`;
  if (
    key.includes("width") ||
    key.includes("radius") ||
  ...
  if (key.includes("resistance") || key.includes("wind"))
...
export function ProductRadarChart({ metrics }: ProductRadarChartProps) {
  ...
  clustersWithMetrics.forEach(([cluster, clusterMetrics]) => {
    ...
    const avgValue =
      clusterMetrics.reduce((sum, m) => sum + m.value, 0) /
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-slate-600">
            Metriche non disponibili per questo prodotto.
          </p>
        </CardContent>
      </Card>
    );
