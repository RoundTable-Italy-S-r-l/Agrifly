        ...
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between gap-4">
          ...
          <nav className="hidden md:flex items-center justify-center flex-1">
            <div className="inline-flex items-center gap-1 rounded-full bg-slate-100/70 px-1.5 py-1 text-[11px] font-semibold tracking-[0.18em] uppercase text-slate-600">
              <Link
                to="/"
                className={`px-3 py-1 rounded-full transition-colors ${isActive('/') ? 'bg-slate-900 text-white' : 'hover:bg-white hover:text-slate-900'}`}
              >
                Home
              </Link>
              <Link
                to="/catalogo"
                className={`px-3 py-1 rounded-full transition-colors ${isActive('/catalogo') ? 'bg-slate-900 text-white' : 'hover:bg-white hover:text-slate-900'}`}
              >
                Catalogo Droni
              </Link>
              <Link
                to="/servizi"
                className={`px-3 py-1 rounded-full transition-colors ${isActive('/servizi') ? 'bg-slate-900 text-white' : 'hover:bg-white hover:text-slate-900'}`}
              >
                Servizi GIS
              </Link>
            </div>
