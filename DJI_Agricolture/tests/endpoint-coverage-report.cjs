...
routeFiles.forEach((file) => {
  ...
  const basePath =
    routeMapping[fileName] ||
    `/api/${fileName.replace("-hono.ts", "").replace(".ts", "")}`;
...
function normalizePathForMatch(path) {
  return path.replace(/^\/api/, "").replace(/:[^/]+/g, ":param");
}
...
allEndpoints.forEach((endpoint) => {
  ...
  const isTested = Array.from(testedEndpoints).some((tested) => {
    ...
    return (
      normalizedEndpointPath === normalizedTestPath ||
      normalizedEndpointPath.replace(/:param/g, ".*") ===
        normalizedTestPath.replace(/:param/g, ".*")
    );
  });
...
console.log(
  `Endpoint mutativi testati: ${mutative.filter((e) => tested.includes(e)).length}`,
);
console.log(
  `Endpoint mutativi non testati: ${mutative.filter((e) => untested.includes(e)).length}`,
);

console.log("\n❌ ENDPOINT MUTATIVI NON TESTATI (PRIORITÀ ALTA):\n");
const untestedMutative = untested.filter((e) =>
