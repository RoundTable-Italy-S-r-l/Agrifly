...
async function testOfferWriteReadBack() {
  ...
  try {
    ...
    if (!createResponse.ok) {
      failed++;
      logTest('Offer creation via API', false, `Status: ${createResponse.status}`);
    ...
    if (dbError || !dbOffer) {
      failed++;
      logTest('Offer read-back from DB', false, dbError?.message || 'Not found');
      return { passed, failed };
    }
    ...
    if (dbOffer.name !== offerData.name) {
      failed++;
      logTest('Offer name consistency', false, `DB: ${dbOffer.name}, API: ${offerData.name}`);
    } else {
      passed++;
    ...
    if (dbOffer.offer_type !== offerData.offer_type) {
      failed++;
      logTest('Offer type consistency', false, `DB: ${dbOffer.offer_type}, API: ${offerData.offer_type}`);
    } else {
      passed++;
    ...
    if (!updateResponse.ok) {
      failed++;
      logTest('Offer update via API', false, `Status: ${updateResponse.status}`);
    } else {
      passed++;
      ...
      if (updatedOffer && updatedOffer.name === updateData.name) {
        ...
        failed++;
        logTest('Offer update consistency', false, 
...
async function testSettingsWriteReadBack() {
  ...
  try {
    ...
    const updateResponse = await fetch(`${API_BASE}/settings/organization/general?orgId=${orgId}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${authTokens.vendor}`
    ...
    if (!updateResponse.ok) {
      failed++;
      logTest('Settings update via API', false, `Status: ${updateResponse.status}`);
      return { passed, failed };
    }
    ...
    if (!updatedOrg) {
      ...
      passed++;
      logTest('Settings read-back consistency', true, `DB: ${updatedOrg.legal_name}`);
    } else {
      failed++;
      logTest('Settings read-back consistency', false, 
...
async function testCatalogToggleWriteReadBack() {
  ...
  if (!authTokens.vendor || !supabase) {
    logTest('Catalog toggle write-read', false, 'Token o Supabase non disponibile');
    return { passed: 0, failed: 1 };
  }
  ...
  try {
    ...
    testInventory = await factory.createInventory(orgId, product.id, { qty_on_hand: 100 });
    testCatalogItem = await factory.createVendorCatalogItem(orgId, testInventory.sku_id, {
      is_for_sale: true
    });
    ...
    const toggleResponse = await fetch(`${API_BASE}/catalog/vendor/${orgId}/toggle`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${authTokens.vendor}`
    ...
    if (!toggleResponse.ok) {
      failed++;
      logTest('Catalog toggle via API', false, `Status: ${toggleResponse.status}`);
      return { passed, failed };
    }
    ...
    if (!updatedItem) {
      ...
      passed++;
      logTest('Catalog toggle read-back consistency', true, `DB: ${updatedItem.is_for_sale}`);
    } else {
      failed++;
      logTest('Catalog toggle read-back consistency', false,
...
async function runAllTests() {
  ...
  const totalPassed = offerResults.passed + settingsResults.passed + catalogResults.passed;
  const totalFailed = offerResults.failed + settingsResults.failed + catalogResults.failed;
