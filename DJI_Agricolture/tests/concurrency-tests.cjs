...
async function testDoubleClickIdempotency() {
  ...
  try {
    ...
    if (!orgId || !supabase) {
      logTest('Toggle double click', false, 'Org ID o Supabase non disponibile');
      return { passed: 0, failed: 1 };
    }
    ...
    const oneConflict = (response1.status === 409 || response2.status === 409) && (response1.ok || response2.ok);

    if (bothOk || oneConflict) {
      passed++;
      logTest('Toggle double click', true, bothOk ? 'Idempotente' : 'Conflitto gestito');
    } else {
      failed++;
      logTest('Toggle double click', false, `Status1: ${response1.status}, Status2: ${response2.status}`);
    }
    ...
    if (finalProduct && finalProduct.is_for_sale === newValue) {
      passed++;
      logTest('Toggle stato finale corretto', true, `is_for_sale: ${finalProduct.is_for_sale}`);
    } else {
      failed++;
  ...
  try {
    ...
    if (finalInventory) {
      ...
      if (available >= 0 && totalReserved >= originalReserved) {
        passed++;
        logTest('Inventory non va sotto zero', true, `Available: ${available}, Reserved: ${totalReserved}`);
      } else {
        failed++;
        logTest('Inventory non va sotto zero', false, `Available: ${available}, Reserved: ${totalReserved}`);
      }

      // Verifica consistenza: qty_reserved non può superare qty_on_hand
      if (totalReserved <= finalInventory.qty_on_hand) {
        passed++;
        logTest('Inventory consistenza reserved', true, `Reserved: ${totalReserved} <= OnHand: ${finalInventory.qty_on_hand}`);
      } else {
        failed++;
        logTest('Inventory consistenza reserved', false, `Reserved: ${totalReserved} > OnHand: ${finalInventory.qty_on_hand}`);
      }
    }
...
async function testMarkAsReadIdempotency() {
  ...
  try {
    ...
    if (response1.status === response2.status) {
      passed++;
      logTest('Mark as read idempotency', true, 'Stesso status su chiamate multiple');
    } else {
      failed++;
      logTest('Mark as read idempotency', false, `Status diversi: ${response1.status} vs ${response2.status}`);
    }
...
async function runAllTests() {
  ...
  const totalPassed = doubleClickResults.passed + inventoryResults.passed + markReadResults.passed;
  const totalFailed = doubleClickResults.failed + inventoryResults.failed + markReadResults.failed;
