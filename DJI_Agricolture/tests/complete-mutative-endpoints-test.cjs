...
const mutativeEndpoints = [
  ...
  // ===== CATALOG =====
  {
    ...
    validBody: async (factory, authTokens) => {
      ...
      const catalogItem = await factory.createVendorCatalogItem(
        orgId,
        inventory.sku_id,
        { is_for_sale: true },
      );
    ...
    verifyWrite: async (db, recordId, writeData) => {
      ...
      if (!data)
        return {
          match: false,
          field: "id",
          expected: writeData._catalogItemId,
          actual: null,
        };
      if (data.is_for_sale !== writeData.isForSale) {
        return {
          match: false,
          field: "is_for_sale",
          expected: writeData.isForSale,
          actual: data.is_for_sale,
        };
      }
      return { match: true };
    },
  ...
  },
  {
    ...
    verifyWrite: async (db, recordId, writeData) => {
      ...
      return data
        ? { match: true }
        : { match: false, field: "id", expected: recordId, actual: null };
    },
  },
  {
  ...
  },
  {
    ...
    verifyWrite: async (db, recordId, writeData) => {
      const { data } = await db
        ...
        .single();
      if (!data)
        return { match: false, field: "id", expected: recordId, actual: null };
      if (data.name !== writeData.name) {
        return {
          match: false,
          field: "name",
          expected: writeData.name,
          actual: data.name,
        };
      }
      return { match: true };
  ...
  // ===== JOBS =====
  {
    ...
    validBody: {
      ...
      target_date_start: new Date(
        Date.now() + 7 * 24 * 60 * 60 * 1000,
      ).toISOString(),
      target_date_end: new Date(
    ...
    verifyWrite: async (db, recordId, writeData) => {
      const { data } = await db
        .from("jobs")
        .select("field_name, service_type")
        .eq("id", recordId)
        .single();
      if (!data)
        return { match: false, field: "id", expected: recordId, actual: null };
  ...
  },
  {
    ...
    verifyWrite: async (db, recordId, writeData) => {
      ...
      const tags =
        typeof data.service_tags === "string"
          ? JSON.parse(data.service_tags)
          : data.service_tags;
  ...
  },
  {
    ...
    validBody: {
      ...
      // Explicitly exclude service_type
    },
    verifyWrite: async (db, recordId, writeData, pathParams) => {
      ...
      const { data } = await db
        .from("rate_cards")
        .select("base_rate_per_ha_cents, id")
        .eq("id", rateCardId)
      ...
      if (data.base_rate_per_ha_cents !== writeData.base_rate_per_ha_cents) {
        return {
          match: false,
          field: "base_rate_per_ha_cents",
          expected: writeData.base_rate_per_ha_cents,
          actual: data.base_rate_per_ha_cents,
        };
      }
      return { match: true };
    },
  ...
  // ===== SERVICES =====
  {
    ...
    verifyWrite: async (db, recordId, writeData) => {
      ...
      const { data } = await db
        .from("rate_cards")
        .select("service_type, base_rate_per_ha_cents")
        .eq("id", recordId)
        .single();
      if (!data)
        return { match: false, field: "id", expected: recordId, actual: null };
      if (data.service_type !== writeData.service_type) {
        return {
          match: false,
          field: "service_type",
          expected: writeData.service_type,
  ...
  },
  {
    ...
    verifyWrite: async (db, recordId, writeData) => {
      // recordId è l'ID del rate card restituito dall'endpoint
      const { data } = await db
        .from("rate_cards")
    ...
    verifyWrite: async (db, recordId, writeData) => {
      ...
      if (parseInt(data.total_cents) !== writeData.total_cents) {
        return {
          match: false,
          field: "total_cents",
          expected: writeData.total_cents,
          actual: data.total_cents,
        };
      }
      return { match: true };
    },
    ...
    verifyWrite: async (db, recordId, writeData, pathParams) => {
      ...
      if (error || !data) {
        return {
          match: false,
          field: "id",
          expected: "message for offer " + offerId,
          actual: null,
        };
      }
      return { match: true };
  ...
  },
  {
    ...
    pathParams: async (factory, authTokens) => {
      ...
      const inviteResponse = await fetch(
        `${API_BASE}/settings/organization/invitations/invite`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  // ===== ORDERS (già testati ma aggiungiamo per completezza) =====
  {
    ...
    validBody: async (factory, authTokens) => {
      ...
      await factory.createVendorCatalogItem(orgId, inventory.sku_id, {
        is_for_sale: true,
      });
      ...
      const cartResponse = await fetch(
        `${API_BASE}/ecommerce/cart?orgId=${buyerOrgId}`,
        {
          headers: { Authorization: `Bearer ${authTokens.buyer}` },
        },
      );
    ...
    try {
      ...
      if (endpoint.requiresAuth && authTokens[endpoint.authType || "vendor"]) {
        headers["Authorization"] =
          `Bearer ${authTokens[endpoint.authType || "vendor"]}`;
      }
    ...
    try {
      ...
      if (response.status === 401 || response.status === 400) {
        ...
        logTest(
          `${endpoint.name} - No auth`,
          true,
          `Status: ${response.status} (OK per path params)`,
        );
      } else {
        endpointFailed++;
        const errorText = await response.text().catch(() => "");
        logTest(
          `${endpoint.name} - No auth`,
          false,
          `Status: ${response.status} (atteso: 401 o 400). Response: ${errorText.substring(0, 80)}`,
        );
      }
    } catch (error) {
      endpointFailed++;
    ...
    try {
      ...
      if (writeResponse.ok) {
        ...
        if (recordId && endpoint.verifyWrite) {
          ...
          const verifyResult =
            endpoint.verifyWrite.length > 3
              ? await endpoint.verifyWrite(
                  supabase,
                  recordId,
                  validBody,
                  pathParams,
...
async function runAllTests() {
  ...
  console.log(
    "Contract Validation + DB Proof per TUTTI gli endpoint mutativi\n",
  );
