...
const tests = [
  { name: '1. API Contract Tests', file: 'api-contract-tests.cjs', critical: true },
  { name: '2. DB Proof Tests', file: 'db-proof-complete.cjs', critical: true },
  { name: '3. Complete Mutative Endpoints', file: 'complete-mutative-endpoints-test.cjs', critical: true },
  { name: '4. Concurrency Tests', file: 'concurrency-tests.cjs', critical: false },
  { name: '5. RLS & Multi-Tenant Tests', file: 'rls-multitenant-tests.cjs', critical: true },
];
...
async function runAll() {
  ...
  console.log(`📈 Tasso successo: ${((passed / results.length) * 100).toFixed(1)}%`);
