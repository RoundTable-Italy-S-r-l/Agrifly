  ...
  if (endpoint.validBody && endpoint.requiresAuth && endpoint.verifyWrite) {
    try {
      ...
      if (writeResponse.ok) {
        ...
        const verifyResult = await endpoint.verifyWrite(
          supabase,
          recordId,
          writeData,
        );
...
const endpoints = [
  ...
  },
  {
    ...
    pathBuilder: (body) =>
      `/settings/organization/general?orgId=${body._orgId}`,
    verifyWrite: async (db, recordId, writeData) => {
      ...
      const { data } = await db
        ...
        .single();
      if (!data)
        return { match: false, field: "id", expected: orgId, actual: null };
      if (data.legal_name !== writeData.legal_name) {
        return {
          match: false,
          field: "legal_name",
          expected: writeData.legal_name,
          actual: data.legal_name,
        };
      }
      return { match: true };
    },
...
async function runAllTests() {
  console.log(
    "🚀 COMPLETE ENDPOINT TEST SUITE - Write + Read-Back Verification\n",
  );
