...
async function testCrossOrgAccess() {
  ...
  if (!authTokens.vendor || !authTokens.buyer || !orgIds.vendor || !orgIds.buyer) {
  ...
  try {
    const response = await fetch(`${API_BASE}/settings/organization/general?orgId=${orgIds.buyer}`, {
      method: 'PATCH',
      headers: {
    ...
    if (response.status === 403 || response.status === 404) {
      passed++;
      logTest('Vendor non può modificare settings buyer', true, `Status: ${response.status}`);
    } else {
      failed++;
      logTest('Vendor non può modificare settings buyer', false, `Status: ${response.status} (atteso: 403 o 404)`);
    }
  } catch (error) {
    failed++;
  ...
  try {
    ...
    const response = await fetch(`${API_BASE}/catalog/vendor/${orgIds.buyer}/product`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${authTokens.vendor}`
    ...
    if (response.status === 403 || response.status === 404) {
      passed++;
      logTest('Vendor non può modificare catalogo buyer', true, `Status: ${response.status}`);
    } else {
      failed++;
      const responseText = await response.text().catch(() => '');
      logTest('Vendor non può modificare catalogo buyer', false, `Status: ${response.status} (atteso: 403 o 404). Response: ${responseText.substring(0, 100)}`);
    }
  } catch (error) {
    failed++;
  ...
  if (supabaseAdmin) {
    try {
      ...
      if (buyerOffer) {
        ...
        const deleteResponse = await fetch(`${API_BASE}/offers/${buyerOffer.id}`, {
          method: 'DELETE',
        ...
        if (deleteResponse.status === 403 || deleteResponse.status === 404) {
          passed++;
          logTest('Vendor non può eliminare offerte buyer', true, `Status: ${deleteResponse.status}`);
        } else {
          failed++;
          logTest('Vendor non può eliminare offerte buyer', false, `Status: ${deleteResponse.status} (atteso: 403 o 404)`);
        }
...
async function testRLSPolicies() {
  ...
  try {
    ...
    if (vendorInventories && vendorInventories.length > 0) {
      ...
      passed++;
      logTest('RLS check - inventario vendor', true, 'Inventario trovato (admin bypass)');
    } else {
      passed++;
      logTest('RLS check - inventario vendor', true, 'Nessun inventario vendor (OK)');
    }
    ...
    if (allOrgs && allOrgs.length === 2) {
      ...
      failed++;
      logTest('RLS check - organizzazioni separate', false, `Trovate: ${allOrgs?.length || 0}`);
    }
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (orgId === orgIds.vendor) {
        ...
        failed++;
        logTest('Token vendor scope corretto', false, `Org ID: ${orgId} (atteso: ${orgIds.vendor})`);
      }
    }
  } catch (error) {
    failed++;
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (orgId === orgIds.buyer) {
        ...
        failed++;
        logTest('Token buyer scope corretto', false, `Org ID: ${orgId} (atteso: ${orgIds.buyer})`);
      }
    }
  } catch (error) {
    failed++;
...
async function runAllTests() {
  ...
  const totalPassed = crossOrgResults.passed + rlsResults.passed + tokenResults.passed;
  const totalFailed = crossOrgResults.failed + rlsResults.failed + tokenResults.failed;
