  ...
  const endpoints = [
    {
      ...
      tests: [
        ...
        { body: { email: 'invalid-email', password: '123' }, expectedStatus: 400, description: 'Email invalida' },
        { body: { email: 'test@test.com', password: '123' }, expectedStatus: [400, 401], description: 'Password troppo corta' },
        { body: { email: 123, password: 'password123' }, expectedStatus: 400, description: 'Email non stringa' },
        { body: { email: 'test@test.com', password: 123 }, expectedStatus: 400, description: 'Password non stringa' },
      ]
    },
    {
      ...
      tests: [
        { body: null, expectedStatus: 400, description: 'Body mancante' },
        { body: {}, expectedStatus: 400, description: 'Body vuoto' },
        { body: { cartId: '', skuId: 'test', quantity: 1 }, expectedStatus: 400, description: 'cartId vuoto' },
        { body: { cartId: 'test', skuId: '', quantity: 1 }, expectedStatus: 400, description: 'skuId vuoto' },
        { body: { cartId: 'test', skuId: 'test', quantity: -1 }, expectedStatus: 400, description: 'Quantity negativa' },
        { body: { cartId: 'test', skuId: 'test', quantity: 0 }, expectedStatus: 400, description: 'Quantity zero' },
        { body: { cartId: 'test', skuId: 'test', quantity: 'invalid' }, expectedStatus: 400, description: 'Quantity non numero' },
      ]
    },
    {
      name: 'POST /offers',
      ...
      tests: [
        { body: null, expectedStatus: 400, description: 'Body mancante' },
        { body: {}, expectedStatus: 400, description: 'Body vuoto' },
        { body: { offer_type: 'INVALID', name: 'Test' }, expectedStatus: 400, description: 'offer_type invalido' },
        { body: { offer_type: 'PROMO', name: '' }, expectedStatus: 400, description: 'Name vuoto' },
        { body: { offer_type: 'PROMO', name: 'Test', valid_from: 'invalid-date' }, expectedStatus: 400, description: 'Date invalida' },
        { body: { offer_type: 'PROMO', name: 'Test', rules_json: 'not-object' }, expectedStatus: 400, description: 'rules_json invalido' },
      ]
    },
    {
      ...
      tests: [
        { body: null, expectedStatus: 400, description: 'Body mancante' },
        { body: {}, expectedStatus: 400, description: 'Body vuoto' },
        { body: { skuId: '' }, expectedStatus: 400, description: 'skuId vuoto' },
        { body: { skuId: 'test', price: -1 }, expectedStatus: 400, description: 'Price negativo' },
        { body: { skuId: 'test', price: 'invalid' }, expectedStatus: 400, description: 'Price non numero' },
      ]
    },
    {
      ...
      tests: [
        { body: null, expectedStatus: 400, description: 'Body mancante' },
        { body: {}, expectedStatus: 400, description: 'Body vuoto' },
        { body: { items: 'not-array' }, expectedStatus: 400, description: 'Items non array' },
        { body: { items: [] }, expectedStatus: 400, description: 'Items vuoto' },
        { body: { items: [{ skuId: '', quantity: 1 }] }, expectedStatus: 400, description: 'skuId vuoto in item' },
        { body: { items: [{ skuId: 'test', quantity: -1 }] }, expectedStatus: 400, description: 'Quantity negativa in item' },
      ]
    },
  ];
  ...
  for (const endpoint of endpoints) {
    for (const test of endpoint.tests) {
      try {
        ...
        if (success) {
          passed++;
          logTest(`${endpoint.name} - ${test.description}`, true, `Status: ${response.status}`);
        } else {
          failed++;
          ...
          logTest(`${endpoint.name} - ${test.description}`, false, errorDetails);
...
async function testResponseValidation() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const noPassword = !data.password && !data.password_hash && !data.hashedPassword;
      const noSecrets = !data.secret && !data.service_role_key;

      if (hasToken && hasUser && noPassword && noSecrets) {
        passed++;
        logTest('Login response validation', true, 'Shape corretta, nessun campo sensibile');
      } else {
        failed++;
        logTest('Login response validation', false, `Missing fields or sensitive data: token=${hasToken}, user=${hasUser}, password=${!noPassword}`);
      }
    }
  } catch (error) {
    failed++;
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (hasId && hasEmail && noPassword && noToken) {
        passed++;
        logTest('Me response validation', true, 'Shape corretta, nessun campo sensibile');
      } else {
        failed++;
        logTest('Me response validation', false, `Issues: id=${hasId}, email=${hasEmail}, password=${!noPassword}`);
      }
    }
  } catch (error) {
    failed++;
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (hasId && hasName && hasType && noSecrets) {
        passed++;
        logTest('Offer creation response validation', true, 'Shape corretta, nessun campo sensibile');
        ...
        failed++;
        logTest('Offer creation response validation', false, `Missing fields or sensitive data`);
      }
    }
  } catch (error) {
    failed++;
...
async function testAuthAndPermissions() {
  ...
  try {
    ...
    if (response.status === 401) {
      ...
      failed++;
      logTest('POST /offers senza token', false, `Status: ${response.status} (atteso: 401)`);
    }
  } catch (error) {
    failed++;
  ...
  try {
    ...
    if (response.status === 401) {
      ...
      failed++;
      logTest('GET /auth/me con token invalido', false, `Status: ${response.status} (atteso: 401)`);
    }
  } catch (error) {
    failed++;
  ...
  try {
    ...
    if (response.status === 401) {
      passed++;
      logTest('GET /auth/me con token scaduto/malformato', true, '401 Unauthorized');
    } else {
      failed++;
      logTest('GET /auth/me con token scaduto/malformato', false, `Status: ${response.status} (atteso: 401)`);
    }
  } catch (error) {
    failed++;
  ...
  if (authTokens.vendor && authTokens.buyer) {
    try {
      ...
      if (meResponse.ok) {
        ...
        const buyerOrgId = buyerData.organization?.id || buyerData.organizationId;

        // Prova a modificare settings con token vendor (altra org)
        const settingsResponse = await fetch(`${API_BASE}/settings/organization/general?orgId=${buyerOrgId}`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${authTokens.vendor}`
        ...
        if (settingsResponse.status === 403 || settingsResponse.status === 404) {
          passed++;
          logTest('Cross-org access prevention', true, `Status: ${settingsResponse.status}`);
        } else {
          failed++;
          logTest('Cross-org access prevention', false, `Status: ${settingsResponse.status} (atteso: 403 o 404)`);
        }
      }
    } catch (error) {
      failed++;
...
async function runAllTests() {
  ...
  const totalPassed = contractResults.passed + responseResults.passed + authResults.passed;
  const totalFailed = contractResults.failed + responseResults.failed + authResults.failed;
